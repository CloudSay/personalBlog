...
<h2 id="小程序-登录" tabindex="-1"><a class="header-anchor" href="#小程序-登录" aria-hidden="true">#</a> 小程序-登录</h2>
<h3 id="unionid和openid" tabindex="-1"><a class="header-anchor" href="#unionid和openid" aria-hidden="true">#</a> unionid和openid</h3>
<p>了解小程序登录之前，我们写了解下小程序/公众号登录涉及到两个最关键的用户标识：</p>
<ul>
...
<h3 id="关键api" tabindex="-1"><a class="header-anchor" href="#关键api" aria-hidden="true">#</a> 关键Api</h3>
<ul>
<li>
<p><a href="https://developers.weixin.qq.com/miniprogram/dev/api/api-login.html" target="_blank" rel="noopener noreferrer"><code>wx.login</code><ExternalLinkIcon/></a> 官方提供的登录能力</p>
</li>
<li>
<p><a href="https://developers.weixin.qq.com/miniprogram/dev/api/signature.html#wxchecksessionobject" target="_blank" rel="noopener noreferrer"><code>wx.checkSession</code><ExternalLinkIcon/></a> 校验用户当前的session_key是否有效</p>
</li>
<li>
<p><a href="https://developers.weixin.qq.com/miniprogram/dev/api/authorize.html" target="_blank" rel="noopener noreferrer"><code>wx.authorize</code><ExternalLinkIcon/></a> 提前向用户发起授权请求</p>
</li>
<li>
<p><a href="https://developers.weixin.qq.com/miniprogram/dev/api/api-login.html" target="_blank" rel="noopener noreferrer"><code>wx.getUserInfo</code><ExternalLinkIcon/></a> 获取用户基本信息</p>
</li>
</ul>
<h3 id="登录流程设计" tabindex="-1"><a class="header-anchor" href="#登录流程设计" aria-hidden="true">#</a> 登录流程设计</h3>
<p>以下从笔者接触过的几种登录流程来做阐述:</p>
<h4 id="利用现有登录体系" tabindex="-1"><a class="header-anchor" href="#利用现有登录体系" aria-hidden="true">#</a> 利用现有登录体系</h4>
<p>直接复用现有系统的登录体系，只需要在小程序端设计用户名，密码/验证码输入页面，便可以简便的实现登录，只需要保持良好的用户体验即可。</p>
<h4 id="利用openid-创建用户体系" tabindex="-1"><a class="header-anchor" href="#利用openid-创建用户体系" aria-hidden="true">#</a> 利用OpenId 创建用户体系</h4>
<p>👆提过，<code>OpenId</code> 是一个小程序对于一个用户的标识，利用这一点我们可以轻松的实现一套基于小程序的用户体系，值得一提的是这种用户体系对用户的打扰最低，可以实现静默登录。具体步骤如下：</p>
<pre><code>1. 小程序客户端通过 `wx.login` 获取 code
...
<h4 id="利用-unionid-创建用户体系" tabindex="-1"><a class="header-anchor" href="#利用-unionid-创建用户体系" aria-hidden="true">#</a> 利用 Unionid 创建用户体系</h4>
<p>如果想实现多个小程序，公众号，已有登录系统的数据互通，可以通过获取到用户 unionid 的方式建立用户体系。因为 unionid 在同一开放平台下的所所有应用都是相同的，通过 <code>unionid</code> 建立的用户体系即可实现全平台数据的互通，更方便的接入原有的功能，那如何获取 <code>unionid</code> 呢，有以下两种方式：</p>
<pre><code>  1. 如果户关注了某个相同主体公众号，或曾经在某个相同主体App、公众号上进行过微信登录授权，通过 `wx.login` 可以直接获取 到 `unionid`
...
<div class="language-text ext-text line-numbers-mode"><pre v-pre class="language-text"><code>   wx.login(获取code) ===> wx.getUserInfo(用户授权) ===> 获取 unionid
</code></pre><div class="line-numbers"><span class="line-number">1</span><br></div></div><ol start="2">
<li>
<p>因为小程序不存在 <code>cookie</code> 的概念， 登录态必须缓存在本地，因此强烈建议为登录态设置过期时间</p>
</li>
<li>
<p>值得一提的是如果需要支持风控安全校验，多平台登录等功能，可能需要加入一些公共参数，例如platform，channel，deviceParam等参数。在和服务端确定方案时，作为前端同学应该及时提出这些合理的建议，设计合理的系统。</p>
</li>
<li>
<p><code>openid</code> ， <code>unionid</code> 不要在接口中明文传输，这是一种危险的行为，同时也很不专业。</p>
</li>
</ol>
<h2 id="小程序-图片导出" tabindex="-1"><a class="header-anchor" href="#小程序-图片导出" aria-hidden="true">#</a> 小程序-图片导出</h2>
<p>经常开发和使用小程序的同学对这个功能一定不陌生，这是一种常见的引流方式，一般同时会在图片中附加一个小程序二维码。</p>
<h3 id="基本原理" tabindex="-1"><a class="header-anchor" href="#基本原理" aria-hidden="true">#</a> 基本原理</h3>
<ol>
<li>
<p>借助 <code>canvas</code> 元素，将需要导出的样式首先在 <code>canvas</code> 画布上绘制出来 （api基本和h5保持一致，但有轻微差异，使用时注意即可）</p>
</li>
<li>
<p>借助微信提供的 <code>canvasToTempFilePath</code> 导出图片，最后再使用 <code>saveImageToPhotosAlbum</code> （需要授权）保存图片到本地</p>
</li>
</ol>
<h3 id="如何优雅实现" tabindex="-1"><a class="header-anchor" href="#如何优雅实现" aria-hidden="true">#</a> 如何优雅实现</h3>
<p>根据上述的原理来看，实现是很简单的，只不过就是设计稿的提取，绘制即可，但是作为一个常用功能，每次都这样写一坨代码岂不是非常的难受。那小程序如何设计一个通用的方法来帮助我们导出图片呢？思路如下：</p>
<ol>
<li>
<p>绘制出需要的样式这一步是省略不掉的。但是我们可以封装一个绘制库，包含常见图形的绘制，例如矩形，圆角矩形，圆， 扇形， 三角形， 文字，图片减少绘制代码，只需要提炼出样式信息，便可以轻松的绘制，最后导出图片存入相册。笔者觉得以下这种方式绘制更为优雅清晰一些，其实也可以使用加入一个type参数来指定绘制类型，传入的一个是样式数组，实现绘制。</p>
</li>
<li>
<p>结合上一步的实现，如果对于同一类型的卡片有多次导出需求的场景，也可以使用自定义组件的方式，封装同一类型的卡片为一个通用组件，在需要导出图片功能的地方，引入该组件即可。</p>
</li>
</ol>
...
</code></pre><div class="line-numbers"><span class="line-number">1</span><br><span class="line-number">2</span><br><span class="line-number">3</span><br><span class="line-number">4</span><br><span class="line-number">5</span><br><span class="line-number">6</span><br><span class="line-number">7</span><br><span class="line-number">8</span><br><span class="line-number">9</span><br><span class="line-number">10</span><br><span class="line-number">11</span><br><span class="line-number">12</span><br><span class="line-number">13</span><br><span class="line-number">14</span><br><span class="line-number">15</span><br><span class="line-number">16</span><br><span class="line-number">17</span><br><span class="line-number">18</span><br><span class="line-number">19</span><br><span class="line-number">20</span><br><span class="line-number">21</span><br><span class="line-number">22</span><br><span class="line-number">23</span><br></div></div><h3 id="注意事项-1" tabindex="-1"><a class="header-anchor" href="#注意事项-1" aria-hidden="true">#</a> 注意事项</h3>
<ol>
<li>小程序中无法绘制网络图片到canvas上，需要通过downLoadFile 先下载图片到本地临时文件才可以绘制</li>
<li>通常需要绘制二维码到导出的图片上，有<a href="https://developers.weixin.qq.com/miniprogram/dev/api/qrcode.html" target="_blank" rel="noopener noreferrer">一种方式<ExternalLinkIcon/></a>导出二维码时，需要携带的参数必须做编码，而且有具体的长度（32可见字符）限制，可以借助服务端生成 <code>短链接</code> 的方式来解决</li>
</ol>
<h2 id="小程序-数据统计" tabindex="-1"><a class="header-anchor" href="#小程序-数据统计" aria-hidden="true">#</a> 小程序-数据统计</h2>
...
<h3 id="设计一个埋点sdk" tabindex="-1"><a class="header-anchor" href="#设计一个埋点sdk" aria-hidden="true">#</a> 设计一个埋点sdk</h3>
<p>小程序的代码结构是，每一个 Page 中都有一个 Page 方法，接受一个包含生命周期函数，数据的 <code>业务逻辑对象</code> 包装这层数据，借助小程序的底层逻辑实现页面的业务逻辑。通过这个我们可以想到思路，对Page进行一次包装，篡改它的生命周期和点击事件，混入埋点代码，不干扰业务逻辑，只要做一些简单的配置即可埋点，简单的代码实现如下：</p>
<div class="language-javascript ext-js line-numbers-mode"><pre v-pre class="language-javascript"><code>  
  代码仅供理解思路
  ...
  <span class="token punctuation">}</span>
</code></pre><div class="line-numbers"><span class="line-number">1</span><br><span class="line-number">2</span><br><span class="line-number">3</span><br><span class="line-number">4</span><br><span class="line-number">5</span><br><span class="line-number">6</span><br><span class="line-number">7</span><br><span class="line-number">8</span><br><span class="line-number">9</span><br><span class="line-number">10</span><br><span class="line-number">11</span><br><span class="line-number">12</span><br><span class="line-number">13</span><br><span class="line-number">14</span><br><span class="line-number">15</span><br><span class="line-number">16</span><br><span class="line-number">17</span><br><span class="line-number">18</span><br><span class="line-number">19</span><br><span class="line-number">20</span><br></div></div><p>这种思路不光适用于埋点，也可以用来作全局异常处理，请求的统一处理等场景。</p>
<h3 id="分析接口" tabindex="-1"><a class="header-anchor" href="#分析接口" aria-hidden="true">#</a> 分析接口</h3>
<p>对于特殊的一些业务，我们可以采取 <code>接口埋点</code>，什么叫接口埋点呢？很多情况下，我们有的api并不是多处调用的，只会在某一个特定的页面调用，通过这个思路我们可以分析出，该接口被请求，则这个行为被触发了，则完全可以通过服务端日志得出埋点数据，但是这种方式局限性较大，而且属于分析结果得出过程，可能存在误差，但可以作为一种思路了解一下。</p>
<h3 id="微信自定义数据分析" tabindex="-1"><a class="header-anchor" href="#微信自定义数据分析" aria-hidden="true">#</a> <a href="https://developers.weixin.qq.com/miniprogram/analysis/index.html?t=18081011" target="_blank" rel="noopener noreferrer">微信自定义数据分析<ExternalLinkIcon/></a></h3>
<p>微信本身提供的数据分析能力，微信本身提供了常规分析和自定义分析两种数据分析方式，在小程序后台配置即可。借助<code>小程序数据助手</code>这款小程序可以很方便的查看。</p>
<h2 id="小程序-工程化" tabindex="-1"><a class="header-anchor" href="#小程序-工程化" aria-hidden="true">#</a> 小程序-工程化</h2>
<h3 id="工程化做什么" tabindex="-1"><a class="header-anchor" href="#工程化做什么" aria-hidden="true">#</a> 工程化做什么</h3>
<p>目前的前端开发过程，工程化是必不可少的一环，那小程序工程化都需要做些什么呢，先看下目前小程序开发当中存在哪些问题需要解决：</p>
<ol>
<li>不支持 css预编译器,作为一种主流的 css解决方案，不论是 less,sass,stylus 都可以提升css效率</li>
...
<p>对于目前常用的工程化方案，webpack，rollup，parcel等来看，都常用与单页应用的打包和处理，而小程序天生是 “多页应用” 并且存在一些特定的配置。根据要解决的问题来看，无非是文件的编译，修改，拷贝这些处理，对于这些需求，我们想到基于流的 <code>gulp</code>非常的适合处理，并且相对于webpack配置多页应用更加简单。所以小程序工程化方案推荐使用 <code>gulp</code></p>
<h3 id="具体开发思路" tabindex="-1"><a class="header-anchor" href="#具体开发思路" aria-hidden="true">#</a> 具体开发思路</h3>
...
<p>上述实现起来其实并不是很难，但是这样的话就是一份纯粹的 gulp 构建脚本和 约定好的目录而已，每次都有一个新的小程序都来拷贝这份脚本来处理吗？显然不合适，那如何真正的实现 <code>小程序工程化</code> 呢？
...
<p>限于篇幅，没有将完整的代码贴上来，如果感兴趣，可以参考笔者公司实现和在生产环境实践过的一整套小程序工程化方案<a href="https://github.com/pandolajs/pandora-cli" target="_blank" rel="noopener noreferrer">pandora-cli<ExternalLinkIcon/></a>。</p>
<h2 id="小程序-持续集成" tabindex="-1"><a class="header-anchor" href="#小程序-持续集成" aria-hidden="true">#</a> 小程序-持续集成</h2>
<p>很多成熟的公司的软件开发流程中为了规范化和保证产品质量，都有 <code>持续集成</code> 这个环节。在小程序这一侧，由于依赖微信开发者平台，和以往的web开发有一定的区别，本节主要介绍如何自动化的做小程序的预览，发布，提审，以实现规范化的开发，上线。</p>
<h3 id="规范化的开发流程" tabindex="-1"><a class="header-anchor" href="#规范化的开发流程" aria-hidden="true">#</a> 规范化的开发流程</h3>
<p>小程序在提审之前，开发者可以通过二维码测试，预览。在这种情况下，如果没有规范化的流程，开发测试流程就会比较混乱，也会存在一些问题(例如不同同学的功能测试，手动提供二维码给测试同学，二维码失效)，所以在开发时笔者建议采用如下开发流程（未接入持续集成）：</p>
<ol>
<li>不同的开发同学根据开发任务拉分支在本地开发，自测</li>
<li>开发完成后提交到远端，经过 <code>review</code> 或者 <code>代码审核</code> 之后，合并到develop分支并上传体验包，作为可提测的版本</li>
<li>告知测试同学可以测试，测试同学可以通过<a href="https://developers.weixin.qq.com/miniprogram/dev/devtools/mydev.html" target="_blank" rel="noopener noreferrer">小程序开发助手<ExternalLinkIcon/></a>打开体验版本来测试</li>
<li>测试完成之后，合并 <code>develop</code> 代码至 <code>master</code> 分支，并提审上线，上线完成后删除无用分支，打上版本<code>tag</code></li>
</ol>
<h3 id="如何做小程序的持续集成" tabindex="-1"><a class="header-anchor" href="#如何做小程序的持续集成" aria-hidden="true">#</a> 如何做小程序的持续集成</h3>
<h4 id="准备工作" tabindex="-1"><a class="header-anchor" href="#准备工作" aria-hidden="true">#</a> 准备工作</h4>
<ol>
<li>操作系统为 <code>windows</code> 或者 <code>macOS</code> 的服务器</li>
<li>服务端安装小程序开发者工具 <a href="https://developers.weixin.qq.com/miniprogram/dev/devtools/download.html" target="_blank" rel="noopener noreferrer">下载地址<ExternalLinkIcon/></a></li>
<li>准备一个通用打包构建角色微信号，并将改微信号添加到所有小程序的开发者中，提供开发者权限</li>
</ol>
<h4 id="开发小程序的集成脚本-可以使用各种语言shell-js-python" tabindex="-1"><a class="header-anchor" href="#开发小程序的集成脚本-可以使用各种语言shell-js-python" aria-hidden="true">#</a> 开发小程序的集成脚本（可以使用各种语言<code>shell</code>, <code>js</code>, <code>python</code>...）</h4>
<p>开发者工具根据提供了 [cli](https://developers.weixin.qq.com/miniprogram/dev/devtools/cli.html 和 <a href="https://developers.weixin.qq.com/miniprogram/dev/devtools/http.html" target="_blank" rel="noopener noreferrer">http<ExternalLinkIcon/></a>两种方式供外部调用来实现登录，预览，上传，下面介绍下 <code>http</code> 调用，<code>cli</code> 方式也很类似。
开发者工具打开之后，本地会启动一个 <code>http</code> 服务，端口号在用户目录的 <code>.ide</code> 文件中，并且提供以下几个接口供开发者调用：</p>
<ul>
<li><code>/open</code> 打开指定路径项目</li>
<li><code>/login</code> 登录</li>
<li><code>/preview</code> 预览指定项目</li>
<li><code>/upload</code> 上传指定项目</li>
<li><code>/test</code> 提交项目</li>
</ul>
...
</code></pre><div class="line-numbers"><span class="line-number">1</span><br><span class="line-number">2</span><br><span class="line-number">3</span><br><span class="line-number">4</span><br><span class="line-number">5</span><br><span class="line-number">6</span><br><span class="line-number">7</span><br><span class="line-number">8</span><br><span class="line-number">9</span><br><span class="line-number">10</span><br><span class="line-number">11</span><br><span class="line-number">12</span><br><span class="line-number">13</span><br><span class="line-number">14</span><br><span class="line-number">15</span><br><span class="line-number">16</span><br><span class="line-number">17</span><br><span class="line-number">18</span><br><span class="line-number">19</span><br><span class="line-number">20</span><br><span class="line-number">21</span><br><span class="line-number">22</span><br><span class="line-number">23</span><br><span class="line-number">24</span><br><span class="line-number">25</span><br></div></div><h4 id="集成" tabindex="-1"><a class="header-anchor" href="#集成" aria-hidden="true">#</a> 集成</h4>
<p>不同公司使用的工具有一些区别，下面简单介绍一下常见的两种(一般是运维同学来执行，笔者不是特别熟悉，所以只是简单介绍)：</p>
<ul>
<li>
<p>gitlab</p>
<p>安装<a href="https://docs.gitlab.com.cn/runner/register/index.html" target="_blank" rel="noopener noreferrer">gitlab runner<ExternalLinkIcon/></a>，搭配gitlab提供的CI
编写CI文件，这份文件会包含构建命令，将上一步编写的脚本集成进执行命令即可</p>
</li>
<li>
<p>Jenkis</p>
<p>安装<a href="https://jenkins.io/download/" target="_blank" rel="noopener noreferrer">Jenkis<ExternalLinkIcon/></a>后新建构建任务，配置任务(指定代码仓库，分支，构建参数)，指明构建方式（可以选择shell，然后编写shell来执行你的脚本）</p>
</li>
</ul>
<p>做完集成后，就可以优化上面介绍的开发流程，将<code>打测试包</code>和<code>发布</code>的权利交给测试同学，开发者安心的开发啦。</p>
<h3 id="总结" tabindex="-1"><a class="header-anchor" href="#总结" aria-hidden="true">#</a> 总结</h3>
<p>以上是笔者实践过的对小程序持续集成的整个流程，不管公司有没有接入持续集成，上面的方式都可以方便开发者自动化的预览，发布，也可以集成到工程化流程中，使用起来会更加方便。具体的实现可以参考<a href="https://github.com/pandolajs/pandora-cli" target="_blank" rel="noopener noreferrer">pandora-cli<ExternalLinkIcon/></a>。</p>
<h2 id="小程序架构" tabindex="-1"><a class="header-anchor" href="#小程序架构" aria-hidden="true">#</a> 小程序架构</h2>
<p><img src="https://yck-1254263422.cos.ap-shanghai.myqcloud.com/blog/2019-06-01-042747.png" alt="architecture"></p>
<p>微信小程序的框架包含两部分 View 视图层、App Service逻辑层。View 层用来渲染页面结构，AppService 层用来逻辑处理、数据请求、接口调用。</p>
<p>它们在<strong>两个线程里</strong>运行。</p>
<p>它们在<strong>两个线程里</strong>运行。</p>
<p>它们在<strong>两个线程里</strong>运行。</p>
<p>视图层和逻辑层通过系统层的 JSBridage 进行通信，逻辑层把数据变化通知到视图层，触发视图层页面更新，视图层把触发的事件通知到逻辑层进行业务处理。</p>
<p>补充</p>
<p><img src="https://yck-1254263422.cos.ap-shanghai.myqcloud.com/blog/2019-06-01-042750.png" alt="one-context"></p>
<p><strong>视图层使用 WebView 渲染，iOS 中使用自带 WKWebView，在 Android 使用腾讯的 x5 内核（基于 Blink）运行。</strong></p>
<p><strong>逻辑层使用在 iOS 中使用自带的 JSCore 运行，在 Android 中使用腾讯的 x5 内核（基于 Blink）运行。</strong></p>
<p><strong>开发工具使用 nw.js 同时提供了视图层和逻辑层的运行环境。</strong></p>
<p>在 Mac下 使用 js-beautify 对微信开发工具 @v1.02.1808080代码批量格式化：</p>
<div class="language-Shell ext-Shell line-numbers-mode"><pre v-pre class="language-Shell"><code>cd /Applications/wechatwebdevtools.app/Contents/Resources/package.nw
find . -type f -name '*.js' -not -path &quot;./node_modules/*&quot; -not -path -exec js-beautify -r -s 2 -p -f '{}' \;
</code></pre><div class="line-numbers"><span class="line-number">1</span><br><span class="line-number">2</span><br></div></div><p>在 <code>js/extensions/appservice/index.js</code> 中找到：</p>
<div class="language-javascript ext-js line-numbers-mode"><pre v-pre class="language-javascript"><code>  <span class="token number">267</span><span class="token operator">:</span> <span class="token keyword">function</span><span class="token punctuation">(</span><span class="token parameter">a<span class="token punctuation">,</span> b<span class="token punctuation">,</span> c</span><span class="token punctuation">)</span> <span class="token punctuation">{</span>
  ...
  <span class="token punctuation">}</span>
</code></pre><div class="line-numbers"><span class="line-number">1</span><br><span class="line-number">2</span><br><span class="line-number">3</span><br><span class="line-number">4</span><br><span class="line-number">5</span><br><span class="line-number">6</span><br><span class="line-number">7</span><br><span class="line-number">8</span><br><span class="line-number">9</span><br><span class="line-number">10</span><br><span class="line-number">11</span><br><span class="line-number">12</span><br><span class="line-number">13</span><br><span class="line-number">14</span><br><span class="line-number">15</span><br><span class="line-number">16</span><br><span class="line-number">17</span><br><span class="line-number">18</span><br><span class="line-number">19</span><br><span class="line-number">20</span><br><span class="line-number">21</span><br><span class="line-number">22</span><br><span class="line-number">23</span><br><span class="line-number">24</span><br><span class="line-number">25</span><br><span class="line-number">26</span><br><span class="line-number">27</span><br><span class="line-number">28</span><br><span class="line-number">29</span><br><span class="line-number">30</span><br><span class="line-number">31</span><br><span class="line-number">32</span><br><span class="line-number">33</span><br><span class="line-number">34</span><br><span class="line-number">35</span><br><span class="line-number">36</span><br><span class="line-number">37</span><br><span class="line-number">38</span><br><span class="line-number">39</span><br><span class="line-number">40</span><br><span class="line-number">41</span><br><span class="line-number">42</span><br><span class="line-number">43</span><br></div></div><p>在 <code>js/extensions/gamenaitveview/index.js</code> 中找到：</p>
<div class="language-javascript ext-js line-numbers-mode"><pre v-pre class="language-javascript"><code>  <span class="token number">299</span><span class="token operator">:</span> <span class="token keyword">function</span><span class="token punctuation">(</span><span class="token parameter">a<span class="token punctuation">,</span> b<span class="token punctuation">,</span> c</span><span class="token punctuation">)</span> <span class="token punctuation">{</span>
    ...
    <span class="token punctuation">}</span>
  <span class="token punctuation">}</span><span class="token punctuation">,</span>
</code></pre><div class="line-numbers"><span class="line-number">1</span><br><span class="line-number">2</span><br><span class="line-number">3</span><br><span class="line-number">4</span><br><span class="line-number">5</span><br><span class="line-number">6</span><br><span class="line-number">7</span><br><span class="line-number">8</span><br><span class="line-number">9</span><br><span class="line-number">10</span><br><span class="line-number">11</span><br><span class="line-number">12</span><br><span class="line-number">13</span><br><span class="line-number">14</span><br><span class="line-number">15</span><br><span class="line-number">16</span><br></div></div><p>在 <code>js/extensions/pageframe/index.js </code>中找到：</p>
<div class="language-javascript ext-js line-numbers-mode"><pre v-pre class="language-javascript"><code><span class="token number">317</span><span class="token operator">:</span> <span class="token keyword">function</span><span class="token punctuation">(</span><span class="token parameter">a<span class="token punctuation">,</span> b<span class="token punctuation">,</span> c</span><span class="token punctuation">)</span> <span class="token punctuation">{</span>
    <span class="token string">'use strict'</span><span class="token punctuation">;</span>
...
<div class="language-javascript ext-js line-numbers-mode"><pre v-pre class="language-javascript"><code><span class="token function">f</span> <span class="token punctuation">(</span><span class="token operator">!</span>r<span class="token punctuation">)</span> p<span class="token punctuation">[</span>b<span class="token punctuation">]</span> <span class="token operator">=</span> s<span class="token punctuation">,</span> f<span class="token punctuation">.</span><span class="token function">send</span><span class="token punctuation">(</span><span class="token punctuation">{</span>
    ...
    <span class="token punctuation">}</span>
<span class="token punctuation">}</span><span class="token punctuation">)</span><span class="token punctuation">;</span>
</code></pre><div class="line-numbers"><span class="line-number">1</span><br><span class="line-number">2</span><br><span class="line-number">3</span><br><span class="line-number">4</span><br><span class="line-number">5</span><br><span class="line-number">6</span><br><span class="line-number">7</span><br><span class="line-number">8</span><br></div></div><p>在 <code>js/extensions/pageframe/index.js</code> 中发现这段代码：</p>
<div class="language-javascript ext-js line-numbers-mode"><pre v-pre class="language-javascript"><code>g<span class="token punctuation">[</span>d<span class="token punctuation">]</span> <span class="token operator">=</span> c<span class="token punctuation">,</span> e<span class="token punctuation">.</span>a<span class="token punctuation">.</span><span class="token function">send</span><span class="token punctuation">(</span><span class="token punctuation">{</span>
...
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers"><span class="line-number">1</span><br><span class="line-number">2</span><br><span class="line-number">3</span><br><span class="line-number">4</span><br><span class="line-number">5</span><br><span class="line-number">6</span><br><span class="line-number">7</span><br><span class="line-number">8</span><br><span class="line-number">9</span><br><span class="line-number">10</span><br><span class="line-number">11</span><br><span class="line-number">12</span><br><span class="line-number">13</span><br><span class="line-number">14</span><br><span class="line-number">15</span><br><span class="line-number">16</span><br><span class="line-number">17</span><br><span class="line-number">18</span><br><span class="line-number">19</span><br><span class="line-number">20</span><br><span class="line-number">21</span><br><span class="line-number">22</span><br><span class="line-number">23</span><br><span class="line-number">24</span><br><span class="line-number">25</span><br><span class="line-number">26</span><br><span class="line-number">27</span><br><span class="line-number">28</span><br><span class="line-number">29</span><br><span class="line-number">30</span><br><span class="line-number">31</span><br><span class="line-number">32</span><br><span class="line-number">33</span><br><span class="line-number">34</span><br><span class="line-number">35</span><br><span class="line-number">36</span><br><span class="line-number">37</span><br><span class="line-number">38</span><br><span class="line-number">39</span><br><span class="line-number">40</span><br><span class="line-number">41</span><br><span class="line-number">42</span><br><span class="line-number">43</span><br><span class="line-number">44</span><br><span class="line-number">45</span><br><span class="line-number">46</span><br><span class="line-number">47</span><br><span class="line-number">48</span><br><span class="line-number">49</span><br></div></div><p>这样的分层设计显然是有意为之的，它的中间层完全控制了程序对于界面进行的操作， 同时对于传递的数据和响应时间也能做到监控。一方面程序的行为受到了极大限制， 另一方面微信可以确保他们对于小程序内容和体验有绝对的控制。</p>
<p>这样的结构也说明了小程序的动画和绘图 API 被设计成生成一个最终对象而不是一步一步执行的样子， 原因就是  Json 格式的数据传递和解析相比与原生 API 都是损耗不菲的，如果频繁调用很可能损耗过多性能，进而影响用户体验。</p>
<h3 id="下载小程序完整包" tabindex="-1"><a class="header-anchor" href="#下载小程序完整包" aria-hidden="true">#</a> 下载小程序完整包</h3>
<p><img src="https://yck-1254263422.cos.ap-shanghai.myqcloud.com/blog/2019-06-01-042751.png" alt="download"></p>
<h3 id="app-service-life-cylce" tabindex="-1"><a class="header-anchor" href="#app-service-life-cylce" aria-hidden="true">#</a> App Service - Life Cylce</h3>
<p><img src="https://yck-1254263422.cos.ap-shanghai.myqcloud.com/blog/2019-06-01-042757.png" alt="lifecycle"></p>
<h3 id="面试题" tabindex="-1"><a class="header-anchor" href="#面试题" aria-hidden="true">#</a> 面试题</h3>
<p><strong>1.动画需要绑定在 data 上，而绘图却不用。你觉得是为什么呢？</strong></p>
<div class="language-javascript ext-js line-numbers-mode"><pre v-pre class="language-javascript"><code><span class="token keyword">var</span> context <span class="token operator">=</span> wx<span class="token punctuation">.</span><span class="token function">createCanvasContext</span><span class="token punctuation">(</span><span class="token string">'firstCanvas'</span><span class="token punctuation">)</span>
...
</code></pre><div class="line-numbers"><span class="line-number">1</span><br><span class="line-number">2</span><br><span class="line-number">3</span><br><span class="line-number">4</span><br><span class="line-number">5</span><br><span class="line-number">6</span><br><span class="line-number">7</span><br><span class="line-number">8</span><br><span class="line-number">9</span><br><span class="line-number">10</span><br><span class="line-number">11</span><br><span class="line-number">12</span><br><span class="line-number">13</span><br><span class="line-number">14</span><br><span class="line-number">15</span><br><span class="line-number">16</span><br><span class="line-number">17</span><br><span class="line-number">18</span><br><span class="line-number">19</span><br></div></div><p><strong>2.小程序的 Http Rquest 请求是不是用的浏览器 Fetch API?</strong></p>
<p>知识点考察</p>
<ul>
<li>知道 Request 是由 Native 实现的</li>
<li>JSCore 是不带 Http Request、Websocket、Storage等功能的，那是 Webkit 带的</li>
<li>小程序的 <code>wx.request</code> 是不是遵循 fetch API 规范实现的呢？答案，显然不是。因为没有 <code>Promise</code></li>
</ul>
<h2 id="view-wxml" tabindex="-1"><a class="header-anchor" href="#view-wxml" aria-hidden="true">#</a> View - WXML</h2>
<p>WXML（WeiXin Markup Language）</p>
<ul>
<li>支持数据绑定</li>
<li>支持逻辑算术、运算</li>
<li>支持模板、引用</li>
<li>支持添加事件（bindtap）</li>
</ul>
<p><img src="https://yck-1254263422.cos.ap-shanghai.myqcloud.com/blog/2019-06-01-042759.jpg" alt="WXML"></p>
<p>Wxml编译器：Wcc  把 Wxml文件 转为 JS</p>
<p>执行方式：Wcc index.wxml</p>
<p><strong>使用 Virtual DOM，进行局部更新</strong></p>
<h2 id="view-wxss" tabindex="-1"><a class="header-anchor" href="#view-wxss" aria-hidden="true">#</a> View - WXSS</h2>
<p>WXSS(WeiXin Style Sheets)</p>
<p><img src="https://yck-1254263422.cos.ap-shanghai.myqcloud.com/blog/2019-06-01-042800.jpg" alt="WXSS"></p>
<p>wxss编译器：wcsc 把wxss文件转化为 js</p>
<p>执行方式： wcsc index.wxss</p>
<h3 id="支持大部分css特性" tabindex="-1"><a class="header-anchor" href="#支持大部分css特性" aria-hidden="true">#</a> 支持大部分CSS特性</h3>
<p>亲测包含但不限于如下内容：</p>
<ul>
<li>Transition</li>
<li>Animation
...
<li>border-radius</li>
<li>calc()</li>
<li>选择器，除了<a href="https://developers.weixin.qq.com/miniprogram/dev/framework/view/wxss.html" target="_blank" rel="noopener noreferrer">官方文档<ExternalLinkIcon/></a>列出的，其实还支持
<ul>
<li>element&gt;element</li>
<li>element+element</li>
<li>element element</li>
<li>element:first-letter</li>
<li>element:first-line</li>
<li>element:first-child</li>
<li>element:last-child</li>
<li>element~element</li>
<li>element:first-of-type</li>
<li>element:last-of-type</li>
<li>element:only-of-type</li>
<li>element:only-child</li>
<li>element:nth-child(n)</li>
<li>element:nth-last-child(n)</li>
<li>element:nth-of-type(n)</li>
<li>element:nth-last-of-type(n)</li>
<li>:root</li>
<li>element:empty</li>
<li>:not(element)</li>
</ul>
...
<p>建议 Css3 的特性都可以做一下尝试。</p>
<h3 id="尺寸单位-rpx" tabindex="-1"><a class="header-anchor" href="#尺寸单位-rpx" aria-hidden="true">#</a> 尺寸单位 rpx</h3>
<p>rpx（responsive pixel）: 可以根据屏幕宽度进行自适应。规定屏幕宽为 750rpx。公式：</p>
<div class="language-Js ext-Js line-numbers-mode"><pre v-pre class="language-Js"><code>const dsWidth = 750
...
<p>可以了解一下 <a href="https://github.com/mpvue/px2rpx-loader" target="_blank" rel="noopener noreferrer">pr2rpx-loader <ExternalLinkIcon/></a>这个库。</p>
<h3 id="样式导入" tabindex="-1"><a class="header-anchor" href="#样式导入" aria-hidden="true">#</a> 样式导入</h3>
<p>使用 <code>@import </code>语句可以导入外联样式表，<code>@import </code>后跟需要导入的外联样式表的相对路径，用 <code>;</code> 表示语句结束。</p>
<h3 id="内联样式" tabindex="-1"><a class="header-anchor" href="#内联样式" aria-hidden="true">#</a> 内联样式</h3>
<p>静态的样式统一写到 class 中。style 接收动态的样式，在运行时会进行解析，<strong>请尽量避免将静态的样式写进 style 中，以免影响渲染速度</strong>。</p>
<h3 id="全局样式与局部样式" tabindex="-1"><a class="header-anchor" href="#全局样式与局部样式" aria-hidden="true">#</a> 全局样式与局部样式</h3>
...
<p><strong>截止20180810</strong></p>
<p>小程序未来有计划支持字体。参考<a href="http://daxue.qq.com/content/content/id/4113" target="_blank" rel="noopener noreferrer">微信公开课<ExternalLinkIcon/></a>。</p>
<p>小程序开发与平时 Web开发类似，也可以使用字体图标，但是 <code>src:url()</code> 无论本地还是远程地址都不行，base64 值则都是可以显示的。</p>
<p>将 ttf 文件转换成 base64。打开这个平台 transfonter.org/。点击 Add fonts 按钮，加载ttf格式的那个文件。将下边的 base64 encode 改为 on。点击 Convert 按钮进行转换，转换后点击 download 下载。</p>
<p>复制下载的压缩文件中的 stylesheet.css 的内容到 font.wxss ，并且将 icomoon 中的 style.css 除了 @font-face 所有的代码也复制到 font.wxss 并将i选择器换成 .iconfont，最后：</p>
...
</code></pre><div class="line-numbers"><span class="line-number">1</span><br></div></div><h2 id="view-component" tabindex="-1"><a class="header-anchor" href="#view-component" aria-hidden="true">#</a> View - Component</h2>
<p>小程序提供了一系列组件用于开发业务功能，按照功能与HTML5的标签进行对比如下：</p>
<p><img src="https://yck-1254263422.cos.ap-shanghai.myqcloud.com/blog/2019-06-01-042801.jpg" alt="Component"></p>
<p>小程序的组件基于Web Component标准</p>
<p>使用Polymer框架实现Web Component</p>
<h2 id="view-native-component" tabindex="-1"><a class="header-anchor" href="#view-native-component" aria-hidden="true">#</a> View - Native Component</h2>
<p>目前Native实现的组件有</p>
<ul>
<li>
<p>canvas</p>
...
<li>
<p>textarea</p>
<p><img src="https://yck-1254263422.cos.ap-shanghai.myqcloud.com/blog/2019-06-01-042802.jpg" alt="Native Component"></p>
</li>
</ul>
<p>Native组件层在 WebView 层之上。这目前带来了一些问题：</p>
<ul>
<li>Native 实现的组件会遮挡其他组件</li>
<li>WebView 渲染出来的视图在滚动时，Native 实现的组件需要更新位置，这会带来性能问题，在安卓机器上比较明显</li>
<li>小程序原生组件 <code>cover-view</code> 可以覆盖 canvas video 等，但是也有一下弊端，比如在 canvas 上覆盖 <code>cover-view</code>，就会发现坐标系不统一处理麻烦</li>
</ul>
<h2 id="目前小程序的问题或限制" tabindex="-1"><a class="header-anchor" href="#目前小程序的问题或限制" aria-hidden="true">#</a> 目前小程序的问题或限制</h2>
<p><strong>截止20180810</strong></p>
<p>包含但不限于：</p>
<ul>
<li>
...
<li>
<p>不能使用 npm，但是可以自搭构建工具或者使用 mpvue。（未来官方有计划支持）</p>
</li>
<li>
...
<li>
<p>小程序不能发朋友圈（可以通过保存图片到本地，发图片到朋友前。二维码可以使用<a href="https://developers.weixin.qq.com/miniprogram/dev/api/qrcode.html" target="_blank" rel="noopener noreferrer">B接口<ExternalLinkIcon/></a>）。</p>
</li>
<li>
<p>获取<a href="https://developers.weixin.qq.com/miniprogram/dev/api/qrcode.html" target="_blank" rel="noopener noreferrer">二维码/小程序<ExternalLinkIcon/></a>接口的限制。</p>
<ul>
<li>B 接口 scene 最大32个可见字符。</li>
<li>AC 接口总共生成的码数量限制为 100,000，请谨慎调用。</li>
<li>真机扫描二维码只能跳转到线上版本，所以测试环境下只可通过开发者工具的通过二维码编译进行调试。</li>
<li>没有发布到线上版本的小程序页面路径会导致生成二维码失败，需要先将添加了页面的小程序发布到线上版本。</li>
</ul>
</li>
<li>
<p>小程序推送只能使用“服务通知” 而且需要用户主动触发提交 formId，formId 只有7天有效期。（现在的做法是在每个页面都放入form并且隐藏以此获取更多的 formId。后端使用原则为：优先使用有效期最短的）</p>
</li>
<li>
<p>小程序大小限制 2M，分包总计不超过 8M</p>
</li>
<li>
<p>转发（分享）小程序不能拿到成功结果，原来可以。<a href="https://mp.weixin.qq.com/s?__biz=MjM5NDAwMTA2MA==&amp;mid=2695730124&amp;idx=1&amp;sn=666a448b047d657350de7684798f48d3&amp;chksm=83d74a07b4a0c311569a748f4d11a5ebcce3ba8f6bd5a4b3183a4fea0b3442634a1c71d3cdd0&amp;scene=21#wechat_redirect" target="_blank" rel="noopener noreferrer">链接<ExternalLinkIcon/></a>（小游戏造的孽）</p>
</li>
<li>
...
<li>
<p>公众号关联小程序，<a href="https://developers.weixin.qq.com/miniprogram/introduction/#%E5%85%AC%E4%BC%97%E5%8F%B7%E5%85%B3%E8%81%94%E5%B0%8F%E7%A8%8B%E5%BA%8F" target="_blank" rel="noopener noreferrer">链接<ExternalLinkIcon/></a></p>
<ul>
<li>所有公众号都可以关联小程序。</li>
...
<li>
<p>安卓系统下，小程序授权获取用户信息之后，删除小程序再重新获取，并重新授权，得到旧签名，导致第一次授权失败</p>
</li>
<li>
<p>开发者工具上，授权获取用户信息之后，如果清缓存选择全部清除，则即使使用了wx.checkSession，并且在session_key有效期内，授权获取用户信息也会得到新的session_key</p>
</li>
</ul>
<h3 id="小程序http2支持情况" tabindex="-1"><a class="header-anchor" href="#小程序http2支持情况" aria-hidden="true">#</a> 小程序HTTP2支持情况</h3>
<h4 id="http2支持情况-模拟器与真机均不支持" tabindex="-1"><a class="header-anchor" href="#http2支持情况-模拟器与真机均不支持" aria-hidden="true">#</a> HTTP2支持情况：模拟器与真机均不支持</h4>
<p>为了验证小程序对HTTP的支持适配情况，我找了两个服务器做测试，一个是网上搜索到支持HTTP2的服务器，一个是我本地起的一个HTTP2服务器。测试中所有请求方法均使用 <code>wx.request</code>。</p>
<ol>
<li>
<p>网上支持HTTP2的服务器：<code>HTTPs://www.snel.com:443</code></p>
</li>
<li>
<p>在Chrome上查看该服务器为 HTTP2</p>
<p><img src="https://yck-1254263422.cos.ap-shanghai.myqcloud.com/blog/2019-06-01-042808.jpg" alt="WechatIMG11"></p>
</li>
<li>
<p>在模拟器上请求该接口，<code>请求头</code>的HTTP版本为HTTP1.1，模拟器不支持HTTP2</p>
<p><img src="https://yck-1254263422.cos.ap-shanghai.myqcloud.com/blog/2019-06-01-042830.jpg" alt="WechatIMG12"></p>
</li>
<li>
<p>由于小程序线上环境需要在项目管理里配置请求域名，而这个域名不是我们需要的请求域名，没必要浪费一个域名位置，所以打开不验证域名，TSL 等选项请求该接口，通过抓包工具表现与模拟器相同</p>
<p><img src="https://yck-1254263422.cos.ap-shanghai.myqcloud.com/blog/2019-06-01-042850.png" alt="WechatIMG14"></p>
</li>
</ol>
<h4 id="http2服务器需要对小程序做兼容性适配" tabindex="-1"><a class="header-anchor" href="#http2服务器需要对小程序做兼容性适配" aria-hidden="true">#</a> HTTP2服务器需要对小程序做兼容性适配</h4>
<p>由上可以看出，在真机与模拟器都不支持 HTTP2，但是都是成功请求的，并且 <code>响应头</code> 里的 HTTP 版本都变成了HTTP1.1 版本，说明服务器对 HTTP1.1 做了兼容性适配。</p>
<ol>
<li>
<p>本地新启一个 node 服务器，返回 JSON 为请求的 HTTP 版本</p>
<p><img src="https://yck-1254263422.cos.ap-shanghai.myqcloud.com/blog/2019-06-01-042908.jpg" alt="WechatIMG16"></p>
</li>
<li>
<p>如果服务器只支持 HTTP2，在模拟器请求时发生了一个 <code>ALPN</code> 协议的错误。并且提醒使用适配 HTTP1</p>
<p><img src="https://yck-1254263422.cos.ap-shanghai.myqcloud.com/blog/2019-06-01-042921.jpg" alt="WechatIMG8"></p>
</li>
<li>
<p>当把服务器的 <code>allowHTTP1</code>，设置为 <code>true</code>，并在请求时处理相关相关请求参数后，模拟器能正常访问接口，并打印出对应的 HTTP 请求版本</p>
<p><img src="https://yck-1254263422.cos.ap-shanghai.myqcloud.com/blog/2019-06-01-042931.jpg" alt="WechatIMG15"></p>
</li>
</ol>
<h2 id="授权获取用户信息流程" tabindex="-1"><a class="header-anchor" href="#授权获取用户信息流程" aria-hidden="true">#</a> 授权获取用户信息流程</h2>
...
<ul>
<li>session_key 有有效期，有效期并没有被告知开发者，只知道用户越频繁使用小程序，session_key 有效期越长</li>
<li>在调用 wx.login 时会直接更新 session_key，导致旧 session_key 失效</li>
<li>小程序内先调用 wx.checkSession 检查登录态，并保证没有过期的 session_key 不会被更新，再调用 wx.login 获取 code。接着用户授权小程序获取用户信息，小程序拿到加密后的用户数据，把加密数据和 code 传给后端服务。后端通过 code 拿到 session_key 并解密数据，将解密后的用户信息返回给小程序</li>
</ul>
...
<ul>
<li>用户授权时，开放平台使用旧的 session_key 对用户信息进行加密。调用 wx.login 重新登录，会刷新 session_key，这时后端服务从开放平台获取到新 session_key，但是无法对老 session_key 加密过的数据解密，用户信息获取失败</li>
<li>在用户信息授权之前先调用 wx.checkSession 呢？wx.checkSession 检查登录态，并且保证 wx.login 不会刷新 session_key，从而让后端服务正确解密数据。但是这里存在一个问题，如果小程序较长时间不用导致 session_key 过期，则 wx.login 必定会重新生成 session_key，从而再一次导致用户信息解密失败。</li>
</ul>
<h2 id="性能优化" tabindex="-1"><a class="header-anchor" href="#性能优化" aria-hidden="true">#</a> 性能优化</h2>
<p><strong>我们知道view部分是运行在webview上的，所以前端领域的大多数优化方式都有用。</strong></p>
<p><strong>我们知道view部分是运行在webview上的，所以前端领域的大多数优化方式都有用。</strong></p>
<p><strong>我们知道view部分是运行在webview上的，所以前端领域的大多数优化方式都有用。</strong></p>
<h3 id="加载优化" tabindex="-1"><a class="header-anchor" href="#加载优化" aria-hidden="true">#</a> 加载优化</h3>
<p><img src="https://yck-1254263422.cos.ap-shanghai.myqcloud.com/blog/2019-06-01-042945.png" alt="preload"></p>
<p>代码包的大小是最直接影响小程序加载启动速度的因素。代码包越大不仅下载速度时间长，业务代码注入时间也会变长。所以最好的优化方式就是减少代码包的大小。</p>
<p><img src="https://yck-1254263422.cos.ap-shanghai.myqcloud.com/blog/2019-06-01-042953.png" alt="load-time-series"></p>
<p>小程序加载的三个阶段的表示。</p>
...
<li>利用缓存: 利用 storage API 对异步请求数据进行缓存，二次启动时先利用缓存数据渲染页面，在进行后台更新。</li>
<li>避免白屏：先展示页面骨架页和基础内容。</li>
<li>及时反馈：即时地对需要用户等待的交互操作给出反馈，避免用户以为小程序无响应。</li>
</ul>
<h4 id="使用分包加载优化" tabindex="-1"><a class="header-anchor" href="#使用分包加载优化" aria-hidden="true">#</a> 使用分包加载优化</h4>
<p><img src="https://yck-1254263422.cos.ap-shanghai.myqcloud.com/blog/2019-06-01-043010.png" alt="sub-package"></p>
<p>在构建小程序分包项目时，构建会输出一个或多个功能的分包，其中每个分包小程序必定含有一个主包，所谓的主包，即放置默认启动页面/TabBar 页面，以及一些所有分包都需用到公共资源/JS 脚本，而分包则是根据开发者的配置进行划分。</p>
...
<p>限制：</p>
<ul>
<li>整个小程序所有分包大小不超过 8M</li>
<li>单个分包/主包大小不能超过 2M</li>
</ul>
<p><strong>原生分包加载的配置</strong>
假设支持分包的小程序目录结构如下：</p>
<div class="language-text ext-text line-numbers-mode"><pre v-pre class="language-text"><code>├── app.js
├── app.json
├── app.wxss
├── packageA
│   └── pages
...
</code></pre><div class="line-numbers"><span class="line-number">1</span><br><span class="line-number">2</span><br><span class="line-number">3</span><br><span class="line-number">4</span><br><span class="line-number">5</span><br><span class="line-number">6</span><br><span class="line-number">7</span><br><span class="line-number">8</span><br><span class="line-number">9</span><br><span class="line-number">10</span><br><span class="line-number">11</span><br><span class="line-number">12</span><br><span class="line-number">13</span><br><span class="line-number">14</span><br><span class="line-number">15</span><br><span class="line-number">16</span><br></div></div><p>开发者通过在 app.json subPackages 字段声明项目分包结构：</p>
...
</code></pre><div class="line-numbers"><span class="line-number">1</span><br><span class="line-number">2</span><br><span class="line-number">3</span><br><span class="line-number">4</span><br><span class="line-number">5</span><br><span class="line-number">6</span><br><span class="line-number">7</span><br><span class="line-number">8</span><br><span class="line-number">9</span><br><span class="line-number">10</span><br><span class="line-number">11</span><br><span class="line-number">12</span><br><span class="line-number">13</span><br><span class="line-number">14</span><br><span class="line-number">15</span><br><span class="line-number">16</span><br><span class="line-number">17</span><br><span class="line-number">18</span><br><span class="line-number">19</span><br><span class="line-number">20</span><br><span class="line-number">21</span><br><span class="line-number">22</span><br></div></div><p><strong>分包原则</strong></p>
<ul>
...
<p><strong>官方即将推出</strong>
分包预加载</p>
<p><img src="https://yck-1254263422.cos.ap-shanghai.myqcloud.com/blog/2019-06-01-043026.png" alt="preload-sub-package"></p>
<p>独立分包</p>
<p><img src="https://yck-1254263422.cos.ap-shanghai.myqcloud.com/blog/2019-06-01-043041.png" alt="single-sub-package"></p>
<h3 id="渲染性能优化" tabindex="-1"><a class="header-anchor" href="#渲染性能优化" aria-hidden="true">#</a> 渲染性能优化</h3>
<p><img src="https://yck-1254263422.cos.ap-shanghai.myqcloud.com/blog/2019-06-01-043047.png" alt="render"></p>
<ul>
<li>
<p>每次 setData 的调用都是一次进程间通信过程，通信开销与 setData 的数据量正相关。</p>
</li>
<li>
<p>setData 会引发视图层页面内容的更新，这一耗时操作一定时间中会阻塞用户交互。</p>
</li>
<li>
...
<ul>
<li>使用 data 在方法间共享数据，<strong>可能增加 setData 传输的数据量</strong>。data 应仅包括与页面渲染相关的数据。</li>
<li>使用 setData 传输大量数据，<strong>通讯耗时与数据正相关，页面更新延迟可能造成页面更新开销增加</strong>。仅传输页面中发生变化的数据，使用 setData 的特殊 key 实现局部更新。</li>
<li>短时间内频繁调用 setData，<strong>操作卡顿，交互延迟，阻塞通信，页面渲染延迟</strong>。避免不必要的 setData，对连续的setData调用进行合并。</li>
<li>在后台页面进行 setData，<strong>抢占前台页面的渲染资源</strong>。页面切入后台后的 setData 调用，延迟到页面重新展示时执行。</li>
</ul>
<p><img src="https://yck-1254263422.cos.ap-shanghai.myqcloud.com/blog/2019-06-01-042750.png" alt="one-context"></p>
<p><strong>避免不当使用onPageScroll</strong></p>
<ul>
<li>只在有必要的时候监听 pageScroll 事件。不监听，则不会派发。</li>
<li>避免在 onPageScroll 中执行复杂逻辑</li>
<li>避免在 onPageScroll 中频繁调用 setData</li>
<li>避免滑动时频繁查询节点信息（SelectQuery）用以判断是否显示，部分场景建议使用节点布局橡胶状态监听（inersectionObserver）替代</li>
</ul>
<p><strong>使用自定义组件</strong></p>
<p>在需要频繁更新的场景下，自定义组件的更新只在组件内部进行，不受页面其他部分内容复杂性影响。</p>
<h2 id="官方小程序技术能力规划" tabindex="-1"><a class="header-anchor" href="#官方小程序技术能力规划" aria-hidden="true">#</a> 官方小程序技术能力规划</h2>
<h3 id="自定义组件2-0" tabindex="-1"><a class="header-anchor" href="#自定义组件2-0" aria-hidden="true">#</a> 自定义组件2.0</h3>
<p>小程序的几个页面间，存在一些相同或是类似的区域，这时候可以把这些区域逻辑封装成一个自定义组件，代码就可以重用，或者对于比较独立逻辑，也可以把它封装成一个自定义组件，也就是微信去年发布的自定义组件，它让代码得到复用、减少代码量，更方便模块化，优化代码架构组织，也使得模块清晰，后期更好地维护，从而保证更好的性能。</p>
<p>但微信打算在原来的基础上推出的自定义组件 2.0，它将拥有更高级的性能：</p>
<ul>
<li>usingComponents 计划支持全局定义和通配符定义：这意味着不用在每个页面反复定义，可以批量导入目录下的所有自定义组件</li>
<li>计划支持类似 Computed 和 watch 的功能，它能使代码逻辑更清晰</li>
<li>计划支持 Component 构造器插件，在实例化一个自定义组件的时候，允许你在构造器的这个阶段，加入一些逻辑，方便进行一些扩展，甚至是可以扩展成 Vue 的语法</li>
</ul>
<h3 id="npm支持" tabindex="-1"><a class="header-anchor" href="#npm支持" aria-hidden="true">#</a> npm支持</h3>
<p>目前小程序开发的痛点是：开源组件要手动复制到项目，后续更新组件也需要手动操作。不久的将来，小程序将支持npm包管理，有了这个之后，想要引入一些开源的项目就变得很简单了，只要在项目里面声明，然后用简单的命令安装，就可以使用了。</p>
<h3 id="官方自定义组件" tabindex="-1"><a class="header-anchor" href="#官方自定义组件" aria-hidden="true">#</a> 官方自定义组件</h3>
<p>微信小程序团队表示，他们在考虑推出一些官方自定义组件，为什么不内置到基础库里呢？因为内置组件要提供给开发者，这个组件一定是开发者很难实现或者是无法实现的一个能力。所以他们更倾向于封装成自定义组件，想基于这些内置组件里，封装一些比较常见的、交互逻辑比较复杂的组件给大家使用，让大家更容易开发。类似弹幕组件，开发者就不用关注弹幕怎么飘，可以节省开发者的开发成本。</p>
<p>同时，他们也想给开发者提供一些规范和一些模板，让开发者设计出好用的自定义组件，更好地被大家去使用。</p>
<h3 id="添加体验评分" tabindex="-1"><a class="header-anchor" href="#添加体验评分" aria-hidden="true">#</a> 添加体验评分</h3>
<p>当小程序加载太慢时，可能会导致用户的流失，而小程序的开发者可能会面临着不知道如何定位问题或不知如何解决问题的困境。</p>
<p>为此，小程序即将推出一个体验评分的功能，这是为了帮助开发者可以检查出小程序有一些什么体验不好的地方，也会同时给出一份优化的指引建议。</p>
<h3 id="原生组件同层渲染" tabindex="-1"><a class="header-anchor" href="#原生组件同层渲染" aria-hidden="true">#</a> 原生组件同层渲染</h3>
<p>小程序在最初的技术选型时，引入了原生组件的概念，因为原生组件可以使小程序的能力更加丰富，比如地图、音视频的能力，但是原生组件是由客户端原生渲染的，导致了原生组件的层级是最高的，开发者很容易遇到打开调试的问题，发现视频组件挡在了 vConsole 上。</p>
<p>为了解决这个问题，当时微信做了一个过渡的方案：cover-view。cover-view可以覆盖在原生组件之上，这一套方案解决了大部分的需求场景。比如说视频组件上很多的按钮、标题甚至还有动画的弹幕，这些都是用 cover-view 去实现的，但它还是没有完全解决原生组件的开发体验问题，因为 cover-view 有一些限制：</p>
<ul>
<li>无法与其他组件混在一起渲染</li>
<li>没有完整的触摸事件</li>
<li>cover-view 对样式的表现有差异</li>
<li>cover-view 对样式的支持度不够高</li>
</ul>
<p>因此微信决定将用同层渲染取代 cover-view，它能像普通组件一样使用，原生组件的层级不再是最高，而是和其他的非原生组件在同一层级渲染，可完全由 z-index 控制，可完全支持触摸事件。</p>
<p>微信表示，同层渲染在 iOS 平台小程序上已经开始内测，会很快开放给开发者，Android 平台已经取得突破性进展，目前正在做一轮封装的工作，开放指日可待。</p>
<h2 id="wepy-vs-mpvue" tabindex="-1"><a class="header-anchor" href="#wepy-vs-mpvue" aria-hidden="true">#</a> wepy vs mpvue</h2>
<h3 id="数据流管理" tabindex="-1"><a class="header-anchor" href="#数据流管理" aria-hidden="true">#</a> 数据流管理</h3>
<p>相比传统的小程序框架，这个一直是我们作为资深开发者比较期望去解决的，在 Web 开发中，随着 Flux、Redux、Vuex 等多个数据流工具出现，我们也期望在业务复杂的小程序中使用。</p>
<ul>
<li>
<p>WePY 默认支持 Redux，在脚手架生成项目的时候可以内置</p>
</li>
<li>
<p>Mpvue 作为 Vue 的移植版本，当然支持 Vuex，同样在脚手架生成项目的时候可以内置</p>
</li>
</ul>
<h3 id="组件化" tabindex="-1"><a class="header-anchor" href="#组件化" aria-hidden="true">#</a> 组件化</h3>
<p>如果你和我们一样，经历了从无到有的小程序业务开发，建议阅读【小程序的组件化开发】章节，进行官方语法的组件库开发（从基础库 1.6.3 开始，官方提供了组件化解决方案）。</p>
<ul>
<li>WePY 类似 Vue 实现了单文件组件，最大的差别是文件后缀 .wpy，只是写法上会有差异，具体可以查看【主流框架使用案例 1：WePY】章节，学习起来有一定成本，不过也会很快适应：</li>
</ul>
<div class="language-text ext-text line-numbers-mode"><pre v-pre class="language-text"><code>export default class Index extends wepy.page {}
</code></pre><div class="line-numbers"><span class="line-number">1</span><br></div></div><ul>
<li>Mpvue 作为 Vue 的移植版本，支持单文件组件，template、script 和 style 都在一个 .vue 文件中，和 vue 的写法类似，所以对 Vue 开发熟悉的同学会比较适应。</li>
</ul>
<h3 id="工程化" tabindex="-1"><a class="header-anchor" href="#工程化" aria-hidden="true">#</a> 工程化</h3>
...
<ul>
<li>wepy 内置了构建，通过 wepy init 命令初始化项目，大致流程如下：</li>
</ul>
<ul>
<li>wepy-cli 会判断模版是在远程仓库还是在本地，如果在本地则会立即跳到第 3 步，反之继续进行。</li>
<li>会从远程仓库下载模版，并保存到本地。</li>
<li>询问开发者 Project name 等问题，依据开发者的回答，创建项目。</li>
</ul>
<ul>
<li>mpvue 沿用了 vue 中推崇的 webpack 作为构建工具，但同时提供了一些自己的插件以及配置文件的一些修改，比如：</li>
</ul>
<ul>
<li>不再需要 html-webpack-plugin</li>
<li>基于 webpack-dev-middleware 修改成 webpack-dev-middleware-hard-disk</li>
<li>最大的变化是基于 webpack-loader 修改成 mpvue-loader</li>
<li>但是配置方式还是类似，分环境配置文件，最终都会编译成小程序支持的目录结构和文件后缀。</li>
</ul>
<h3 id="综合比较" tabindex="-1"><a class="header-anchor" href="#综合比较" aria-hidden="true">#</a> 综合比较</h3>
<table>
<thead>
<tr>
<th>对比\框架</th>
...
<h3 id="选型的个人看法" tabindex="-1"><a class="header-anchor" href="#选型的个人看法" aria-hidden="true">#</a> 选型的个人看法</h3>
<p>先说结论：选择 mpvue。</p>
<p>wepy vs mpvue。</p>
<p>理由：</p>
<p><strong>工程化</strong>
原生开发因为不带工程化，诸如NPM包（未来会引入）、ES7、图片压缩、PostCss、pug、ESLint等等不能用。如果自己要搭工程化，不如直接使用wepy或mpvue。mpvue和wepy都可以和小程序原生开发混写。<a href="#https://github.com/mpvue/examples/tree/master/echarts">参考mpvue-echart</a>，<a href="https://github.com/Tencent/wepy/issues/1560" target="_blank" rel="noopener noreferrer">参考wepy<ExternalLinkIcon/></a>。
而问题在于wepy没有引入webpack(wepy@2.0.x依然没有引入)，以上说的这些东西都要造轮子（作者造或自己造）。没有引入 Webpack 是一个重大的硬伤。社区维护的成熟 Webpack 显然更稳定，轮子更多。</p>
<p><strong>维护</strong>
wepy 也是社区维护的，是官方的？其实 wepy 的主要开发者只有作者一人，附上一个<a href="https://github.com/Tencent/wepy/graphs/contributors" target="_blank" rel="noopener noreferrer">contrubutors<ExternalLinkIcon/></a>链接。另外被官方招安了也是后来的事，再说腾讯要有精力帮着一起维护好 wepy，为什么不花精力在小程序原生开发上呢？再来看看 mpvue，是美团一个前端小组维护的。</p>
<p><strong>学习成本</strong>
Vue 的学习曲线比较平缓。mpvue 是 Vue的子集。所以 mpvue 的学习成本会低于 wepy。尤其是之前技术栈有学过用过 Vue 的。</p>
<p><strong>未来规划</strong>
mpvue 已经支持 web 和小程序。因为 mpvue 基于AST，所以未来可以支持支付宝小程序和快应用。他们也是有这样的规划。</p>
<p>请在需求池下面自己找
<img src="https://yck-1254263422.cos.ap-shanghai.myqcloud.com/blog/2019-06-01-043107.jpg" alt="mpvue-feature"></p>
<p><strong>坑</strong>
两者都有各自的坑。但是我觉得有一些wepy的坑是没法容忍的。比如<a href="https://github.com/Tencent/wepy/issues/1231" target="_blank" rel="noopener noreferrer">repeat组建里面用computed得到的列表全是同一套数据<ExternalLinkIcon/></a>而且1.x是没法解决的。
wepy和mpvue我都开发过完整小程序的体验下，我觉得wepy的坑更多，而且wepy有些坑碍于架构设计没办法解决。</p>
<h2 id="mpvue" tabindex="-1"><a class="header-anchor" href="#mpvue" aria-hidden="true">#</a> mpvue</h2>
<blockquote>
<p>Vue.js 小程序版, fork 自 vuejs/vue@2.4.1，保留了 vue runtime 能力，添加了小程序平台的支持。
<code>mpvue</code> 是一个使用 <code>Vue.js</code> 开发小程序的前端框架。框架基于 <code>Vue.js</code> 核心，<code>mpvue</code> 修改了 <code>Vue.js</code> 的 runtime 和 compiler 实现，使其可以运行在小程序环境中，从而为小程序开发引入了整套 <code>Vue.js</code> 开发体验。</p>
</blockquote>
<h3 id="框架原理" tabindex="-1"><a class="header-anchor" href="#框架原理" aria-hidden="true">#</a> 框架原理</h3>
<p><strong>两个大方向</strong></p>
<ul>
<li>通过<code>mpvue</code>提供 mp 的 runtime 适配小程序</li>
<li>通过<code>mpvue-loader</code>产出微信小程序所需要的文件结构和模块内容。</li>
</ul>
<p><strong>七个具体问题</strong></p>
<p>要了解 mpvue 原理必然要了解 Vue 原理，这是大前提。但是要讲清楚 Vue 原理需要花费大量的篇幅，不如参考<a href="https://github.com/answershuto/learnVue" target="_blank" rel="noopener noreferrer">learnVue<ExternalLinkIcon/></a>。</p>
<p>现在假设您对 Vue 原理有个大概的了解。</p>
<p>由于 Vue 使用了 Virtual DOM，所以 Virtual DOM 可以在任何支持 JavaScript 语言的平台上操作，譬如说目前 Vue 支持浏览器平台或 weex，也可以是 mp(小程序)。那么最后 Virtual DOM 如何映射到真实的 DOM 节点上呢？vue为平台做了一层适配层，浏览器平台见 <a href="https://github.com/Meituan-Dianping/mpvue/blob/master/src/platforms/web/runtime/node-ops.js" target="_blank" rel="noopener noreferrer">runtime/node-ops.js<ExternalLinkIcon/></a>、weex平台见<a href="https://github.com/Meituan-Dianping/mpvue/blob/master/src/platforms/weex/runtime/node-ops.js" target="_blank" rel="noopener noreferrer">runtime/node-ops.js<ExternalLinkIcon/></a>，小程序见<a href="https://github.com/Meituan-Dianping/mpvue/blob/master/src/platforms/mp/runtime/node-ops.js" target="_blank" rel="noopener noreferrer">runtime/node-ops.js<ExternalLinkIcon/></a>。不同平台之间通过适配层对外提供相同的接口，Virtual DOM进行操作Real DOM节点的时候，只需要调用这些适配层的接口即可，而内部实现则不需要关心，它会根据平台的改变而改变。</p>
<p>所以思路肯定是往增加一个 mp 平台的 runtime 方向走。但问题是小程序不能操作 DOM，所以 mp 下的<code>node-ops.js</code> 里面的实现都是直接 <code>return obj</code>。</p>
<p>新 Virtual DOM 和旧 Virtual DOM 之间需要做一个 patch，找出 diff。patch完了之后的 diff 怎么更新视图，也就是如何给这些 DOM 加入 attr、class、style 等 DOM 属性呢？ Vue 中有 nextTick 的概念用以更新视图，mpvue这块对于小程序的 <code>setData</code> 应该怎么处理呢？</p>
<p>另外个问题在于小程序的 Virtual DOM 怎么生成？也就是怎么将 template 编译成<code>render function</code>。这当中还涉及到<a href="https://cn.vuejs.org/v2/guide/installation.html#%E8%BF%90%E8%A1%8C%E6%97%B6-%E7%BC%96%E8%AF%91%E5%99%A8-vs-%E5%8F%AA%E5%8C%85%E5%90%AB%E8%BF%90%E8%A1%8C%E6%97%B6" target="_blank" rel="noopener noreferrer">运行时-编译器-vs-只包含运行时<ExternalLinkIcon/></a>，显然如果要提高性能、减少包大小、输出 wxml、mpvue 也要提供预编译的能力。因为要预输出 wxml 且没法动态改变 DOM，所以动态组件，自定义 render，和<code>&lt;script type=&quot;text/x-template&quot;&gt;</code> 字符串模版等都不支持(<a href="http://mpvue.com/mpvue/#_15" target="_blank" rel="noopener noreferrer">参考<ExternalLinkIcon/></a>)。</p>
<p>另外还有一些其他问题，最后总结一下</p>
<ul>
<li>1.如何预编译生成<code>render function</code></li>
<li>2.如何预编译生成 wxml，wxss，wxs</li>
<li>3.如何 patch 出 diff</li>
<li>4.如何更新视图</li>
<li>5.如何建立小程序事件代理机制，在事件代理函数中触发与之对应的vue组件事件响应</li>
<li>6.如何建立vue实例与小程序 Page 实例关联</li>
<li>7.如何建立小程序和vue生命周期映射关系，能在小程序生命周期中触发vue生命周期</li>
</ul>
<p><strong><a href="(https://github.com/Meituan-Dianping/mpvue/tree/master/src/platforms/mp)">platform/mp的目录结构</a></strong></p>
<div class="language-text ext-text line-numbers-mode"><pre v-pre class="language-text"><code>.
...
</code></pre><div class="line-numbers"><span class="line-number">1</span><br><span class="line-number">2</span><br><span class="line-number">3</span><br><span class="line-number">4</span><br><span class="line-number">5</span><br><span class="line-number">6</span><br><span class="line-number">7</span><br></div></div><p><strong>后面的内容逐步解答这几个问题，也就弄明白了原理</strong></p>
<h4 id="mpvue-loader" tabindex="-1"><a class="header-anchor" href="#mpvue-loader" aria-hidden="true">#</a> mpvue-loader</h4>
<p><a href="https://github.com/mpvue/mpvue-loader" target="_blank" rel="noopener noreferrer">mpvue-loader<ExternalLinkIcon/></a> 是 <a href="https://github.com/vuejs/vue-loader" target="_blank" rel="noopener noreferrer">vue-loader<ExternalLinkIcon/></a> 的一个扩展延伸版，类似于超集的关系，除了<a href="https://github.com/vuejs/vue-loader" target="_blank" rel="noopener noreferrer">vue-loader<ExternalLinkIcon/></a> 本身所具备的能力之外，它还会利用<a href="https://github.com/Meituan-Dianping/mpvue/tree/master/packages/mpvue-template-compiler" target="_blank" rel="noopener noreferrer">mpvue-template-compiler<ExternalLinkIcon/></a>生成<code>render function</code>。</p>
<ul>
<li>entry</li>
</ul>
<p>它会从 <code>webpack</code> 的配置中的 entry 开始，分析依赖模块，并分别打包。在entry 中 app 属性及其内容会被打包为微信小程序所需要的 app.js／app.json／app.wxss，其余的会生成对应的页面page.js/page.json/page.wxml/page.wxss，如示例的 entry 将会生成如下这些文件，文件内容下文慢慢讲来：</p>
...
<li>wxml
每一个 <code>.vue</code> 的组件都会被生成为一个 wxml 规范的 template，然后通过 wxml 规范的 import 语法来达到一个复用，同时组件如果涉及到 props 的 data 数据，我们也会做相应的处理，举个实际的例子：</li>
</ul>
...
</code></pre><div class="line-numbers"><span class="line-number">1</span><br><span class="line-number">2</span><br><span class="line-number">3</span><br><span class="line-number">4</span><br><span class="line-number">5</span><br><span class="line-number">6</span><br><span class="line-number">7</span><br><span class="line-number">8</span><br><span class="line-number">9</span><br><span class="line-number">10</span><br><span class="line-number">11</span><br><span class="line-number">12</span><br><span class="line-number">13</span><br><span class="line-number">14</span><br><span class="line-number">15</span><br><span class="line-number">16</span><br><span class="line-number">17</span><br><span class="line-number">18</span><br><span class="line-number">19</span><br></div></div><p>这样一个 Vue 的组件的模版部分会生成相应的 wxml</p>
<div class="language-html ext-html line-numbers-mode"><pre v-pre class="language-html"><code><span class="token tag"><span class="token tag"><span class="token punctuation">&lt;</span>import</span> <span class="token attr-name">src</span><span class="token attr-value"><span class="token punctuation attr-equals">=</span><span class="token punctuation">"</span>components/other-component$hash.wxml<span class="token punctuation">"</span></span> <span class="token punctuation">/></span></span>
...
<div class="language-html ext-html line-numbers-mode"><pre v-pre class="language-html"><code><span class="token tag"><span class="token tag"><span class="token punctuation">&lt;</span>template</span> <span class="token attr-name">is</span><span class="token attr-value"><span class="token punctuation attr-equals">=</span><span class="token punctuation">"</span>other-component$hash<span class="token punctuation">"</span></span> <span class="token attr-name">data</span><span class="token attr-value"><span class="token punctuation attr-equals">=</span><span class="token punctuation">"</span>{{ ...$c[0] }}<span class="token punctuation">"</span></span><span class="token punctuation">></span></span><span class="token tag"><span class="token tag"><span class="token punctuation">&lt;/</span>template</span><span class="token punctuation">></span></span>
</code></pre><div class="line-numbers"><span class="line-number">1</span><br></div></div><p>mpvue 在运行时会从根组件开始把所有的组件实例数据合并成一个树形的数据，然后通过 setData 到 appData,<code>$c </code>是 $children 的缩写。至于那个 0 则是我们的 compiler 处理过后的一个标记，会为每一个子组件打一个特定的不重复的标记。 树形数据结构如下：</p>
<div class="language-javascript ext-js line-numbers-mode"><pre v-pre class="language-javascript"><code><span class="token comment">// 这儿数据结构是一个数组，index 是动态的</span>
<span class="token punctuation">{</span>
...
<p>这个部分的处理同 web 的处理差异不大，唯一不同在于通过配置生成 .css 为 .wxss ，其中的对于 css 的若干处理，在 postcss-mpvue-wxss 和 px2rpx-loader 这两部分的文档中又详细的介绍。</p>
<p>app.json／page.json
1.1.1 以上</p>
<p>推荐和小程序一样，将 app.json／page.json 放到页面入口处，使用 copy-webpack-plugin copy 到对应的生成位置。</p>
<p>1.1.1 以下</p>
<p>这部分内容来源于 app 和 page 的 entry 文件，通常习惯是 main.js，你需要在你的入口文件中 export default { config: {} }，这才能被我们的 loader 识别为这是一个配置，需要写成 json 文件。</p>
...
<span class="token keyword">export</span> <span class="token keyword">default</span> <span class="token punctuation">{</span>
    ...
    <span class="token punctuation">}</span>
<span class="token punctuation">}</span><span class="token punctuation">;</span>
</code></pre><div class="line-numbers"><span class="line-number">1</span><br><span class="line-number">2</span><br><span class="line-number">3</span><br><span class="line-number">4</span><br><span class="line-number">5</span><br><span class="line-number">6</span><br><span class="line-number">7</span><br><span class="line-number">8</span><br><span class="line-number">9</span><br><span class="line-number">10</span><br><span class="line-number">11</span><br><span class="line-number">12</span><br><span class="line-number">13</span><br><span class="line-number">14</span><br><span class="line-number">15</span><br><span class="line-number">16</span><br><span class="line-number">17</span><br><span class="line-number">18</span><br><span class="line-number">19</span><br></div></div><p>同时，这个时候，我们会根据 entry 的页面数据，自动填充到 app.json 中的 pages 字段。 pages 字段也是可以自定义的，约定带有 ^ 符号开头的页面，会放到数组的最前面。</p>
<p>style scoped
在 vue-loader 中对 style scoped 的处理方式是给每个样式加一个 attr 来标记 module-id，然后在 css 中也给每条 rule 后添加 [module-id]，最终可以形成一个 css 的“作用域空间”。</p>
<p>在微信小程序中目前是不支持 attr 选择器的，所以我们做了一点改动，把 attr 上的 [module-id] 直接写到了 class 里，如下：</p>
<div class="language-html ext-html line-numbers-mode"><pre v-pre class="language-html"><code><span class="token comment">&lt;!-- .vue --></span>
<span class="token tag"><span class="token tag"><span class="token punctuation">&lt;</span>template</span><span class="token punctuation">></span></span>
...
<p>生产出的内容是：</p>
<div class="language-javascript ext-js line-numbers-mode"><pre v-pre class="language-javascript"><code><span class="token punctuation">(</span><span class="token keyword">function</span><span class="token punctuation">(</span><span class="token parameter">module<span class="token punctuation">,</span> __webpack_exports__<span class="token punctuation">,</span> __webpack_require__</span><span class="token punctuation">)</span> <span class="token punctuation">{</span>
...
</span></code></pre><div class="line-numbers"><span class="line-number">1</span><br><span class="line-number">2</span><br><span class="line-number">3</span><br><span class="line-number">4</span><br><span class="line-number">5</span><br><span class="line-number">6</span><br><span class="line-number">7</span><br><span class="line-number">8</span><br><span class="line-number">9</span><br><span class="line-number">10</span><br><span class="line-number">11</span><br><span class="line-number">12</span><br><span class="line-number">13</span><br><span class="line-number">14</span><br><span class="line-number">15</span><br><span class="line-number">16</span><br><span class="line-number">17</span><br><span class="line-number">18</span><br><span class="line-number">19</span><br><span class="line-number">20</span><br><span class="line-number">21</span><br><span class="line-number">22</span><br><span class="line-number">23</span><br><span class="line-number">24</span><br><span class="line-number">25</span><br><span class="line-number">26</span><br><span class="line-number">27</span><br><span class="line-number">28</span><br><span class="line-number">29</span><br><span class="line-number">30</span><br></div></div><h4 id="compiler" tabindex="-1"><a class="header-anchor" href="#compiler" aria-hidden="true">#</a> compiler</h4>
<p>compiler相关，也就是template预编译这块，可以参考《<a href="https://github.com/answershuto/learnVue/blob/master/docs/%E8%81%8A%E8%81%8AVue%E7%9A%84template%E7%BC%96%E8%AF%91.MarkDown#createcompiler" target="_blank" rel="noopener noreferrer">聊聊Vue的template编译<ExternalLinkIcon/></a>》来搞明白。原理是一样的。</p>
<p>mpvue自己实现了<code>export { compile, compileToFunctions, compileToWxml }</code>(<a href="https://github.com/Meituan-Dianping/mpvue/blob/master/src/platforms/mp/compiler/index.js" target="_blank" rel="noopener noreferrer">链接<ExternalLinkIcon/></a>)其中<code>compileToWxml</code>是用来生成wxml，具体代码<a href="https://github.com/mpvue/mpvue-loader/blob/master/lib/mp-compiler/index.js#L30" target="_blank" rel="noopener noreferrer">在这<ExternalLinkIcon/></a>。</p>
<p>另外mpvue是不需要提供<a href="https://cn.vuejs.org/v2/guide/installation.html#%E8%BF%90%E8%A1%8C%E6%97%B6-%E7%BC%96%E8%AF%91%E5%99%A8-vs-%E5%8F%AA%E5%8C%85%E5%90%AB%E8%BF%90%E8%A1%8C%E6%97%B6" target="_blank" rel="noopener noreferrer">运行时-编译器<ExternalLinkIcon/></a>的，虽然理论上是能够做到的。因为小程序不能操作DOM，即便提供了运行时-编译器也产生不了界面。</p>
<p>详细讲解compile过程：</p>
<p>1.将vue文件解析成模板对象</p>
<div class="language-javascript ext-js line-numbers-mode"><pre v-pre class="language-javascript"><code><span class="token comment">// mpvue-loader/lib/loader.js</span>
...
</code></pre><div class="line-numbers"><span class="line-number">1</span><br><span class="line-number">2</span><br></div></div><p>假如vue文件源码如下:</p>
...
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers"><span class="line-number">1</span><br><span class="line-number">2</span><br><span class="line-number">3</span><br><span class="line-number">4</span><br><span class="line-number">5</span><br><span class="line-number">6</span><br><span class="line-number">7</span><br><span class="line-number">8</span><br><span class="line-number">9</span><br><span class="line-number">10</span><br><span class="line-number">11</span><br><span class="line-number">12</span><br><span class="line-number">13</span><br><span class="line-number">14</span><br><span class="line-number">15</span><br><span class="line-number">16</span><br><span class="line-number">17</span><br><span class="line-number">18</span><br><span class="line-number">19</span><br><span class="line-number">20</span><br><span class="line-number">21</span><br><span class="line-number">22</span><br><span class="line-number">23</span><br><span class="line-number">24</span><br><span class="line-number">25</span><br><span class="line-number">26</span><br><span class="line-number">27</span><br><span class="line-number">28</span><br><span class="line-number">29</span><br><span class="line-number">30</span><br><span class="line-number">31</span><br></div></div><p>2.调用mpvue-loader/lib/template-compiler/index.js导出的接口并传入上面得到的html模板：</p>
<div class="language-javascript ext-js line-numbers-mode"><pre v-pre class="language-javascript"><code><span class="token keyword">var</span> templateCompilerPath <span class="token operator">=</span> normalize<span class="token punctuation">.</span><span class="token function">lib</span><span class="token punctuation">(</span><span class="token string">'template-compiler/index'</span><span class="token punctuation">)</span>
<span class="token operator">...</span>
...
<div class="language-javascript ext-js line-numbers-mode"><pre v-pre class="language-javascript"><code><span class="token comment">// mpvue-loader/lib/template-compiler/index.js</span>
...
</code></pre><div class="line-numbers"><span class="line-number">1</span><br><span class="line-number">2</span><br></div></div><p>compile方法生产下面的ast(Abstract Syntax Tree)模板，render函数和staticRenderFns</p>
<div class="language-javascript ext-js line-numbers-mode"><pre v-pre class="language-javascript"><code><span class="token punctuation">{</span>
...
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers"><span class="line-number">1</span><br><span class="line-number">2</span><br><span class="line-number">3</span><br><span class="line-number">4</span><br><span class="line-number">5</span><br><span class="line-number">6</span><br><span class="line-number">7</span><br><span class="line-number">8</span><br><span class="line-number">9</span><br><span class="line-number">10</span><br><span class="line-number">11</span><br><span class="line-number">12</span><br><span class="line-number">13</span><br><span class="line-number">14</span><br><span class="line-number">15</span><br><span class="line-number">16</span><br><span class="line-number">17</span><br><span class="line-number">18</span><br><span class="line-number">19</span><br><span class="line-number">20</span><br><span class="line-number">21</span><br><span class="line-number">22</span><br><span class="line-number">23</span><br><span class="line-number">24</span><br><span class="line-number">25</span><br><span class="line-number">26</span><br><span class="line-number">27</span><br><span class="line-number">28</span><br><span class="line-number">29</span><br><span class="line-number">30</span><br><span class="line-number">31</span><br><span class="line-number">32</span><br><span class="line-number">33</span><br><span class="line-number">34</span><br><span class="line-number">35</span><br><span class="line-number">36</span><br><span class="line-number">37</span><br><span class="line-number">38</span><br><span class="line-number">39</span><br><span class="line-number">40</span><br><span class="line-number">41</span><br><span class="line-number">42</span><br><span class="line-number">43</span><br><span class="line-number">44</span><br><span class="line-number">45</span><br><span class="line-number">46</span><br><span class="line-number">47</span><br><span class="line-number">48</span><br><span class="line-number">49</span><br><span class="line-number">50</span><br><span class="line-number">51</span><br><span class="line-number">52</span><br><span class="line-number">53</span><br><span class="line-number">54</span><br><span class="line-number">55</span><br><span class="line-number">56</span><br><span class="line-number">57</span><br><span class="line-number">58</span><br><span class="line-number">59</span><br><span class="line-number">60</span><br><span class="line-number">61</span><br><span class="line-number">62</span><br><span class="line-number">63</span><br><span class="line-number">64</span><br><span class="line-number">65</span><br><span class="line-number">66</span><br><span class="line-number">67</span><br><span class="line-number">68</span><br><span class="line-number">69</span><br><span class="line-number">70</span><br><span class="line-number">71</span><br><span class="line-number">72</span><br><span class="line-number">73</span><br><span class="line-number">74</span><br><span class="line-number">75</span><br><span class="line-number">76</span><br><span class="line-number">77</span><br><span class="line-number">78</span><br><span class="line-number">79</span><br><span class="line-number">80</span><br><span class="line-number">81</span><br><span class="line-number">82</span><br><span class="line-number">83</span><br><span class="line-number">84</span><br><span class="line-number">85</span><br><span class="line-number">86</span><br><span class="line-number">87</span><br><span class="line-number">88</span><br><span class="line-number">89</span><br><span class="line-number">90</span><br><span class="line-number">91</span><br><span class="line-number">92</span><br><span class="line-number">93</span><br><span class="line-number">94</span><br><span class="line-number">95</span><br><span class="line-number">96</span><br><span class="line-number">97</span><br><span class="line-number">98</span><br><span class="line-number">99</span><br><span class="line-number">100</span><br><span class="line-number">101</span><br><span class="line-number">102</span><br></div></div><p>其中的render函数运行的结果是返回<code>VNode</code>对象，其实<code>render</code>函数应该长下面这样：</p>
<div class="language-javascript ext-js line-numbers-mode"><pre v-pre class="language-javascript"><code><span class="token punctuation">(</span><span class="token keyword">function</span><span class="token punctuation">(</span><span class="token punctuation">)</span> <span class="token punctuation">{</span>
  ...
  <span class="token punctuation">}</span>
<span class="token punctuation">}</span><span class="token punctuation">)</span>
</code></pre><div class="line-numbers"><span class="line-number">1</span><br><span class="line-number">2</span><br><span class="line-number">3</span><br><span class="line-number">4</span><br><span class="line-number">5</span><br><span class="line-number">6</span><br><span class="line-number">7</span><br><span class="line-number">8</span><br><span class="line-number">9</span><br><span class="line-number">10</span><br><span class="line-number">11</span><br><span class="line-number">12</span><br><span class="line-number">13</span><br><span class="line-number">14</span><br><span class="line-number">15</span><br><span class="line-number">16</span><br></div></div><p>其中的<code>_c</code>就是vue对象的<code>createElement</code>方法 (创建元素)，<code>_m</code>是<code>renderStatic</code>（渲染静态节点），<code>_v</code> 是
<code>createTextVNode</code>（创建文本dom），<code>_s</code> 是 <code>toString</code> （转换为字符串）</p>
<div class="language-javascript ext-js line-numbers-mode"><pre v-pre class="language-javascript"><code><span class="token comment">// src/core/instance/render.js</span>
...
</code></pre><div class="line-numbers"><span class="line-number">1</span><br><span class="line-number">2</span><br></div></div><p><strong>以上解答了问题1、2</strong></p>
<h4 id="runtime" tabindex="-1"><a class="header-anchor" href="#runtime" aria-hidden="true">#</a> runtime</h4>
<p><a href="(https://github.com/Meituan-Dianping/mpvue/tree/master/src/platforms/mp/runtime)">目录结构</a></p>
<div class="language-text ext-text line-numbers-mode"><pre v-pre class="language-text"><code>.
├── events.js //解答问题5
...
├── patch.js //解答问题3
└── render.js //解答问题4
</code></pre><div class="line-numbers"><span class="line-number">1</span><br><span class="line-number">2</span><br><span class="line-number">3</span><br><span class="line-number">4</span><br><span class="line-number">5</span><br><span class="line-number">6</span><br><span class="line-number">7</span><br></div></div><p><strong><a href="https://github.com/Meituan-Dianping/mpvue/blob/master/src/platforms/mp/runtime/patch.js" target="_blank" rel="noopener noreferrer">patch.js<ExternalLinkIcon/></a></strong></p>
<p>和vue使用的<code>createPatchFunction</code>保持一致，任然是旧树和新树进行patch产出diff，但是多了一行this.$updateDataToMP()用以更新。</p>
<p><strong><a href="https://github.com/Meituan-Dianping/mpvue/blob/master/src/platforms/mp/runtime/render.js" target="_blank" rel="noopener noreferrer">render.js<ExternalLinkIcon/></a></strong></p>
<p>两个核心的方法<code>initDataToMP</code>、<code>updateDataToMP</code>。</p>
<p><code>initDataToMP</code>收集vm上的data，然后调用小程序Page示例的<code>setData</code>渲染。</p>
<p><code>updateDataToMP</code>在每次patch，也就是依赖收集发现数据改变时更新(参考patch.js代码)，这部分一样会使用<code>nextTick</code>和队列。最终使用了节流阀<code>throttleSetData</code>。50毫秒用来控制频率以解决频繁修改Data，会造成大量传输Data数据而导致的性能问题。</p>
<p>其中<code>collectVmData</code>最终也是用到了<code>formatVmData</code>。尤其要注意的是一句注释：</p>
<blockquote>
<p>getVmData 这儿获取当前组件内的所有数据，包含 props、computed 的数据</p>
</blockquote>
<p>我们又知道，service到view是两个线程间通信，如果Data含有大量数据，增加了传输数据量，加大了传输成本，将会造成性能下降。</p>
<p><strong><a href="https://github.com/Meituan-Dianping/mpvue/blob/master/src/platforms/mp/runtime/events.js" target="_blank" rel="noopener noreferrer">events.js<ExternalLinkIcon/></a></strong></p>
<p>正如官网所说的，这里使用<code>eventTypeMap</code>做了各<a href="http://mpvue.com/mpvue/#_13" target="_blank" rel="noopener noreferrer">事件的隐射<ExternalLinkIcon/></a></p>
<div class="language-javascript ext-js line-numbers-mode"><pre v-pre class="language-javascript"><code><span class="token keyword">import</span> <span class="token punctuation">{</span> getComKey<span class="token punctuation">,</span> eventTypeMap <span class="token punctuation">}</span> <span class="token keyword">from</span> <span class="token string">'../util/index'</span>
</code></pre><div class="line-numbers"><span class="line-number">1</span><br></div></div><div class="language-javascript ext-js line-numbers-mode"><pre v-pre class="language-javascript"><code><span class="token comment">// 用于小程序的 event type 到 web 的 event</span>
...
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers"><span class="line-number">1</span><br><span class="line-number">2</span><br><span class="line-number">3</span><br><span class="line-number">4</span><br><span class="line-number">5</span><br><span class="line-number">6</span><br><span class="line-number">7</span><br><span class="line-number">8</span><br><span class="line-number">9</span><br><span class="line-number">10</span><br><span class="line-number">11</span><br><span class="line-number">12</span><br><span class="line-number">13</span><br><span class="line-number">14</span><br><span class="line-number">15</span><br><span class="line-number">16</span><br></div></div><p>使用了<code>handleProxyWithVue</code>方法来代理小程序事件到vue事件。</p>
<p>另外看下作者自己对这部分的<a href="https://tech.meituan.com/mt_mpvue_development_framework.html" target="_blank" rel="noopener noreferrer">思路<ExternalLinkIcon/></a></p>
<blockquote>
...
<p><code>getHandle</code>这个方法应该就是作者思路当中所说的：找到对应节点，然后找到handle。</p>
<p><strong><a href="https://github.com/Meituan-Dianping/mpvue/blob/master/src/platforms/mp/runtime/lifecycle.js" target="_blank" rel="noopener noreferrer">lifecycle.js<ExternalLinkIcon/></a></strong></p>
<p>在<code>initMP</code>方法中，自己创建小程序的App、Page。实现生命周期相关方法，使用<code>callHook</code>代理兼容小程序App、Page的生命周期。</p>
<p><a href="http://mpvue.com/mpvue/#_4" target="_blank" rel="noopener noreferrer">官方文档生命周期<ExternalLinkIcon/></a>中说到了：</p>
<blockquote>
<p>同 vue，不同的是我们会在小程序 onReady 后，再去触发 vue mounted 生命周期</p>
</blockquote>
<p>这部分查看，<code>onReady</code>之后才会执行<code>next</code>，这个<code>next</code>回调最终是vue的<code>mountComponent</code>。可以在<a href="https://github.com/Meituan-Dianping/mpvue/blob/master/src/platforms/mp/runtime/index.js#L37" target="_blank" rel="noopener noreferrer">index.js<ExternalLinkIcon/></a>中看到。这部分代码也就是解决了&quot;小程序生命周期中触发vue生命周期&quot;。</p>
<div class="language-javascript ext-js line-numbers-mode"><pre v-pre class="language-javascript"><code><span class="token keyword">export</span> <span class="token keyword">function</span> <span class="token function">initMP</span> <span class="token punctuation">(</span><span class="token parameter">mpType<span class="token punctuation">,</span> next</span><span class="token punctuation">)</span> <span class="token punctuation">{</span>
  <span class="token comment">// ...</span>
  ...
  <span class="token comment">// ...</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers"><span class="line-number">1</span><br><span class="line-number">2</span><br><span class="line-number">3</span><br><span class="line-number">4</span><br><span class="line-number">5</span><br><span class="line-number">6</span><br><span class="line-number">7</span><br><span class="line-number">8</span><br><span class="line-number">9</span><br><span class="line-number">10</span><br><span class="line-number">11</span><br><span class="line-number">12</span><br><span class="line-number">13</span><br></div></div><p>在小程序onShow时，使用$nextTick去第一次渲染数据，参考上面提到的render.js。</p>
<div class="language-javascript ext-js line-numbers-mode"><pre v-pre class="language-javascript"><code><span class="token keyword">export</span> <span class="token keyword">function</span> <span class="token function">initMP</span> <span class="token punctuation">(</span><span class="token parameter">mpType<span class="token punctuation">,</span> next</span><span class="token punctuation">)</span> <span class="token punctuation">{</span>
  <span class="token comment">// ...</span>
  global<span class="token punctuation">.</span><span class="token function">Page</span><span class="token punctuation">(</span><span class="token punctuation">{</span>
...
<div class="language-javascript ext-js line-numbers-mode"><pre v-pre class="language-javascript"><code><span class="token keyword">export</span> <span class="token keyword">function</span> <span class="token function">initMP</span> <span class="token punctuation">(</span><span class="token parameter">mpType<span class="token punctuation">,</span> next</span><span class="token punctuation">)</span> <span class="token punctuation">{</span>
  ...
  <span class="token comment">// ...</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers"><span class="line-number">1</span><br><span class="line-number">2</span><br><span class="line-number">3</span><br><span class="line-number">4</span><br><span class="line-number">5</span><br><span class="line-number">6</span><br><span class="line-number">7</span><br><span class="line-number">8</span><br><span class="line-number">9</span><br></div></div><p><strong><a href="https://github.com/Meituan-Dianping/mpvue/blob/master/src/platforms/mp/runtime/index.js" target="_blank" rel="noopener noreferrer">index.js<ExternalLinkIcon/></a></strong></p>
<p>最后index.js就负责各种初始化和mount。</p>
<h3 id="class和style为什么暂不支持组件" tabindex="-1"><a class="header-anchor" href="#class和style为什么暂不支持组件" aria-hidden="true">#</a> Class和Style为什么暂不支持组件</h3>
<p>原因：目前的组件是使用小程序的 template 标签实现的，给组件指定的class和style是挂载在template标签上，而template 标签不支持 class 及 style 属性。</p>
<p>解决方案： 在自定义组件上绑定class或style到一个props属性上。</p>
<div class="language-html ext-html line-numbers-mode"><pre v-pre class="language-html"><code> // 组件ComponentA.vue
 <span class="token tag"><span class="token tag"><span class="token punctuation">&lt;</span>template</span><span class="token punctuation">></span></span>
...
<span class="token function">data</span> <span class="token punctuation">(</span><span class="token punctuation">)</span> <span class="token punctuation">{</span>
  ...
  <span class="token punctuation">}</span>
<span class="token operator">&lt;</span><span class="token operator">/</span>script<span class="token operator">></span>
</code></pre><div class="line-numbers"><span class="line-number">1</span><br><span class="line-number">2</span><br><span class="line-number">3</span><br><span class="line-number">4</span><br><span class="line-number">5</span><br><span class="line-number">6</span><br><span class="line-number">7</span><br></div></div><div class="language-css ext-css line-numbers-mode"><pre v-pre class="language-css"><code>&lt;style lang=<span class="token string">"stylus"</span> scoped>
  .a-class
    border red solid 2rpx
  .b-class
    margin-right 20rpx
&lt;/style>
</code></pre><div class="line-numbers"><span class="line-number">1</span><br><span class="line-number">2</span><br><span class="line-number">3</span><br><span class="line-number">4</span><br><span class="line-number">5</span><br><span class="line-number">6</span><br></div></div><p>但是这样会有问题就是style加上scoped之后，编译模板生成的代码是下面这样的：</p>
<div class="language-css ext-css line-numbers-mode"><pre v-pre class="language-css"><code> <span class="token selector">.a-class.data-v-8f1d914e</span> <span class="token punctuation">{</span>
...
<li>在定义组件上绑定style属性到一个props属性上：</li>
</ul>
<div class="language-html ext-html line-numbers-mode"><pre v-pre class="language-html"><code> <span class="token comment">&lt;!--P组件ComponentA.vue--></span>
 <span class="token tag"><span class="token tag"><span class="token punctuation">&lt;</span>template</span><span class="token punctuation">></span></span>
...
<span class="token operator">&lt;</span><span class="token operator">/</span>script<span class="token operator">></span>
  ...
  ...
&lt;/style>
</code></pre><div class="line-numbers"><span class="line-number">1</span><br><span class="line-number">2</span><br><span class="line-number">3</span><br></div></div><p>也可以通过定义styleObject，然后通过工具函数转化为styleString，如下所示：</p>
<div class="language-javascript ext-js line-numbers-mode"><pre v-pre class="language-javascript"><code><span class="token keyword">const</span> bstyle <span class="token operator">=</span> <span class="token punctuation">{</span>
...
<li>当然自定义组件确定只会改变某个css样式，通过pros传入单个样式的值，然后通过:style绑定肯定没问题：</li>
</ul>
<div class="language-html ext-html line-numbers-mode"><pre v-pre class="language-html"><code><span class="token comment">&lt;!--组件ComponentA.vue--></span>
 <span class="token tag"><span class="token tag"><span class="token punctuation">&lt;</span>template</span><span class="token punctuation">></span></span>
...
<span class="token tag"><span class="token tag"><span class="token punctuation">&lt;/</span>template</span><span class="token punctuation">></span></span>
</code></pre><div class="line-numbers"><span class="line-number">1</span><br><span class="line-number">2</span><br><span class="line-number">3</span><br><span class="line-number">4</span><br></div></div><h3 id="分包加载" tabindex="-1"><a class="header-anchor" href="#分包加载" aria-hidden="true">#</a> 分包加载</h3>
<p>package.json修改</p>
<ul>
<li>升级： &quot;mpvue-loader&quot;: &quot;^1.1.2-rc.4&quot; &quot;webpack-mpvue-asset-plugin&quot;: &quot;^0.1.1&quot;</li>
<li>新增： &quot;relative&quot;: &quot;^3.0.2&quot;</li>
</ul>
<p>注意事项</p>
<ul>
<li>1.1.2-rc.5 修复 slot 文件路径生成错误的问题</li>
<li>1.1.x 版本还不是很稳定，对稳定性要求较高的项目建议暂时使用 1.0.x 版本</li>
</ul>
...
</code></pre><div class="line-numbers"><span class="line-number">1</span><br><span class="line-number">2</span><br><span class="line-number">3</span><br><span class="line-number">4</span><br></div></div><p>to</p>
<div class="language-javascript ext-js line-numbers-mode"><pre v-pre class="language-javascript"><code><span class="token punctuation">{</span>
...
<ul>
<li>本次升级意在调整生成文件目录结构，对依赖的文件由原来的写死绝对路径该改为相对路径</li>
<li>mpvue-loader@1.1.2-rc.4 依赖 webpack-mpvue-asset-plugin@0.1.0 做依赖资源引用</li>
...
</code></pre><div class="line-numbers"><span class="line-number">1</span><br><span class="line-number">2</span><br><span class="line-number">3</span><br><span class="line-number">4</span><br><span class="line-number">5</span><br><span class="line-number">6</span><br><span class="line-number">7</span><br><span class="line-number">8</span><br><span class="line-number">9</span><br><span class="line-number">10</span><br><span class="line-number">11</span><br><span class="line-number">12</span><br><span class="line-number">13</span><br><span class="line-number">14</span><br><span class="line-number">15</span><br><span class="line-number">16</span><br><span class="line-number">17</span><br><span class="line-number">18</span><br><span class="line-number">19</span><br><span class="line-number">20</span><br></div></div><h3 id="问题与展望" tabindex="-1"><a class="header-anchor" href="#问题与展望" aria-hidden="true">#</a> 问题与展望</h3>
<p>技术的更新迭代是很快的，很多内容在写的时候还是这样。过了几天就发生了变化。又仔细看了小程序的文档，发现小程序原生开发深受vue影响啊，越来越像了。</p>
<p>希望mpvue能够使用<code>wx.nextTick</code><a href="https://developers.weixin.qq.com/miniprogram/dev/api/custom-component.html#wxnexttickfunction" target="_blank" rel="noopener noreferrer">链接<ExternalLinkIcon/></a>，尝试来代替50毫秒</p>
<p>希望能够解决<a href="https://github.com/Meituan-Dianping/mpvue/issues/639" target="_blank" rel="noopener noreferrer">使用脏检查优化每次更新数据时都会传输大量数据的问题, 解决删除回退, 列表忽然滚动到顶部等问题<ExternalLinkIcon/></a>。也许可以靠下面的自定义组件。</p>
<p>使用<a href="https://developers.weixin.qq.com/miniprogram/dev/framework/custom-component/" target="_blank" rel="noopener noreferrer">自定义组件<ExternalLinkIcon/></a>代替template，这样可以解决诸如:</p>
<ul>
<li><a href="http://mpvue.com/mpvue/#_10" target="_blank" rel="noopener noreferrer">组件根标签不能使用style和class<ExternalLinkIcon/></a></li>
<li><a href="https://github.com/Meituan-Dianping/mpvue/issues?utf8=%E2%9C%93&amp;q=slot" target="_blank" rel="noopener noreferrer">slot的各种问题<ExternalLinkIcon/></a></li>
<li><a href="http://mpvue.com/mpvue/#vue_1" target="_blank" rel="noopener noreferrer">Slot（scoped 暂时还没做支持）<ExternalLinkIcon/></a></li>
<li>setData的性能提升，因为官方说的:&quot;在需要频繁更新的场景下，自定义组件的更新只在组件内部进行，不受页面其他部分内容复杂性影响。&quot;。也就是说，组件内部的setData只会影响组件范围。这个和Vue就很像了，我觉得原理肯定是一致的。</li>
</ul>
<p>在小程序完善了自定义组件之后，我现在的倾向变成了自搭或者网上找脚手架来工程化项目，使用诸如：NPM、PostCSS、pug、babel、ESLint、图片优化等功能。然后使用小程序原生开发的方式来开发，因为它做的越来越好，越来越像vue了。</p>
<h2 id="小程序-学习" tabindex="-1"><a class="header-anchor" href="#小程序-学习" aria-hidden="true">#</a> 小程序-学习</h2>
<p>经常看到一些同学在查找小程序的学习资料和面对一些问题时无从下手。这一节笔者会基于自己的经验告诉大家如何学习开发小程序和如何解决遇到的问题。</p>
<h3 id="学习建议" tabindex="-1"><a class="header-anchor" href="#学习建议" aria-hidden="true">#</a> 学习建议</h3>
<ol>
<li>
<p><strong>文档一定要通读</strong>，<strong>文档一定要通读</strong>，<strong>文档一定要通读</strong>。如果你想轻松的实现各种功能，先不要去搜网上的各种二手资料，请一定要熟读文档。不是为了记下来，而是有个大概印象，知道小程序有哪些能力和限制。下面笔者列出一些文档和社区里很优质的内容（首页的就不列了），虽然在很显眼的位置，但是很多人没看过🤦。(个人感觉，平时很多开发者问的问题，百分之九十都在文档里有答案)</p>
<ul>
<li><a href="https://developers.weixin.qq.com/community/develop/list/512" target="_blank" rel="noopener noreferrer">小程序-小故事<ExternalLinkIcon/></a> 可以了解小程序的发展和对一些功能的权衡取舍过程</li>
<li><a href="https://developers.weixin.qq.com/community/develop/list/4" target="_blank" rel="noopener noreferrer">小程序基础教程<ExternalLinkIcon/></a> 非常好和全面的教程，墙裂推荐</li>
<li><a href="https://developers.weixin.qq.com/community/develop/list/2" target="_blank" rel="noopener noreferrer">官方公告<ExternalLinkIcon/></a>  小程序仍然在快速迭代当中，了解官方的动态对于开发者非常有必要</li>
<li><a href="https://developers.weixin.qq.com/community/develop/buglist" target="_blank" rel="noopener noreferrer">小程序已知问题和修复日志<ExternalLinkIcon/></a> 躲坑指南</li>
<li><a href="https://developers.weixin.qq.com/miniprogram/product/" target="_blank" rel="noopener noreferrer">运营规范&amp;常见拒绝情形<ExternalLinkIcon/></a>  小程序宪法</li>
<li><a href="https://developers.weixin.qq.com/miniprogram/dev/api/authorize-index.html" target="_blank" rel="noopener noreferrer">需要授权的操作<ExternalLinkIcon/></a> 授权是有可能流失用户的，要多注意</li>
<li><a href="https://mp.weixin.qq.com/wiki?t=resource/res_main&amp;id=mp1421140183" target="_blank" rel="noopener noreferrer">微信公众平台技术文档<ExternalLinkIcon/></a> 涉及到和公众号交互等内容可能会用到</li>
<li><a href="https://open.weixin.qq.com/cgi-bin/showdocument?action=dir_list&amp;t=resource/res_list&amp;verify=1&amp;id=1417674108&amp;token=&amp;lang=zh_CN" target="_blank" rel="noopener noreferrer">微信开放平台技术文档<ExternalLinkIcon/></a> 涉及和App交互等内容可能会用到</li>
</ul>
</li>
<li>
<p>利用好微信提供的各种辅助工具和能力</p>
<ul>
<li><a href="https://developers.weixin.qq.com/miniprogram/dev/devtools/mydev.html" target="_blank" rel="noopener noreferrer">小程序开发助手<ExternalLinkIcon/></a> 方便查看开发版，体验版，线上版</li>
<li><a href="https://developers.weixin.qq.com/miniprogram/dev/demo.html" target="_blank" rel="noopener noreferrer">小程序示例<ExternalLinkIcon/></a> 直观的了解小程序的各种能力，新手可以当做demo跑一下</li>
<li><a href="https://developers.weixin.qq.com/miniprogram/analysis/assistant/" target="_blank" rel="noopener noreferrer">小程序数据助手<ExternalLinkIcon/></a> 查看小程序pv，uv，方便了解自己的工作成果</li>
<li>利用好小程序提供的<a href="https://developers.weixin.qq.com/miniprogram/dev/devtools/debug.html" target="_blank" rel="noopener noreferrer">调试能力<ExternalLinkIcon/></a> 现在小程序已经有了真机调试，各种特殊场景的测试(扫码，支付)，已经非常全面。</li>
</ul>
</li>
<li>
<p>看完文档直接上手开发，多动手喽，干就完了!</p>
</li>
</ol>
<p>建议的进阶路线:</p>
<pre><code>1. 熟读文档 
...
<h3 id="如何解决遇到的问题" tabindex="-1"><a class="header-anchor" href="#如何解决遇到的问题" aria-hidden="true">#</a> 如何解决遇到的问题</h3>
<p>由于小程序本身的技术架构，开发技能和web技术共性很多，我们之前在web开发中的很多开发经验也是有效的，大多数问题也是很好解决的(看文档)。常见的一些问题：</p>
<ol>
<li>
<p>兼容性问题</p>
<p>一般遇到设备兼容性问题，从以下几个角度思考:</p>
<ol>
<li>不通的微信版本的<a href="https://developers.weixin.qq.com/miniprogram/dev/framework/client-lib.html" target="_blank" rel="noopener noreferrer">小程序基础库<ExternalLinkIcon/></a>是不一样的，很多 <code>API</code> 是有基础库的要求。</li>
<li>样式写法问题，小程序提供了rpx单位让样式开发更简单，但如果单位混用的话可能会导致意想不到的效果。</li>
<li>css兼容性问题 例如ios7，8对 <code>flex布局</code> 支持的不够好。</li>
<li>设备兼容问题，例如ios不支持 <code>webp</code> 图片格式。</li>
</ol>
</li>
<li>
<p>代码逻辑问题</p>
<p>对于平常的业务逻辑来说一般都是跟小程序的<a href="https://developers.weixin.qq.com/miniprogram/dev/framework/app-service/page.html" target="_blank" rel="noopener noreferrer">生命周期<ExternalLinkIcon/></a>挂钩，如果业务逻辑复杂，代码组织能力不够，很容易将代码写的很乱，这样出现问题的风险自然也会很高。
所以建议在开发复杂业务逻辑时，一定要先借助流程图，思维脑图等方式分析清楚业务，然后再规划代码逻辑，拆分出逻辑主次再开发。
遇到问题时，推荐如下方法:</p>
<ol>
<li><code>debugger</code> 大法，在出现问题的地方打上断点，一步步查看上下文中的变量异常</li>
<li><code>二分删代码大法</code> 遇到极其诡异（注意是极其诡异）的问题时，二分删代码直到问题消失，定位到问题代码(悄悄告诉你们，笔者用这个方法帮同事定位到好几个诡异的问题)</li>
</ol>
</li>
<li>
<p>性能问题</p>
<p>官方提供的<a href="https://developers.weixin.qq.com/miniprogram/dev/framework/performance/" target="_blank" rel="noopener noreferrer">性能优化工具，和文档<ExternalLinkIcon/></a>为第一资料，毕竟他们自己写的坑，自己最清楚。另外上面已经介绍过一些性能优化的方法了，参照业务场景对症下药即可。</p>
</li>
<li>
<p>各种奇怪的问题</p>
<p>事实上，90%的问题还是不看文档导致的，所以你们懂得。另外一些问题参考上述也可以定位到，如果还有不能解决的问题怎么办？
那可能是微信的<code>bug</code>，所以去已知问题文档，官方社区翻一翻，一般都有蛛丝马迹。实在不行也可以向老司机提问，注意提问姿势，最好提供最小可复现demo，代码片段功能了解一下</p>
</li>
</ol>
...
<p>以上内容部分来自：</p>
<ul>
<li><a href="https://zhuanlan.zhihu.com/p/22754296" target="_blank" rel="noopener noreferrer">微信小程序架构分析 (上)<ExternalLinkIcon/></a></li>
<li><a href="https://zhuanlan.zhihu.com/p/25105936" target="_blank" rel="noopener noreferrer">微信小程序架构解析<ExternalLinkIcon/></a></li>
<li><a href="http://daxue.qq.com/content/online/id/4107" target="_blank" rel="noopener noreferrer">2018微信公开课第七季上海站·小程序专场<ExternalLinkIcon/></a></li>
<li><a href="https://juejin.im/entry/5a54b73b6fb9a01ca7135335" target="_blank" rel="noopener noreferrer">小程序中使用iconfont<ExternalLinkIcon/></a></li>
<li><a href="http://www.infoq.com/cn/news/2018/07/wchat-miniprog-support" target="_blank" rel="noopener noreferrer">微信小程序的下一步：支持NPM、小程序云、可视化编程、支持分包<ExternalLinkIcon/></a></li>
<li><a href="http://mpvue.com/build/mpvue-loader/" target="_blank" rel="noopener noreferrer">mpvue-docs<ExternalLinkIcon/></a></li>
<li><a href="https://juejin.im/post/5afd836251882567105ff8b4" target="_blank" rel="noopener noreferrer">使用Mpvue开发微信小程序的最佳实践<ExternalLinkIcon/></a></li>
<li><a href="https://tech.meituan.com/mt_mpvue_development_framework.html" target="_blank" rel="noopener noreferrer">用Vue.js开发微信小程序：开源框架mpvue解析<ExternalLinkIcon/></a></li>
<li><a href="https://github.com/answershuto/learnVue" target="_blank" rel="noopener noreferrer">learnVue<ExternalLinkIcon/></a></li>
</ul>
</template>
